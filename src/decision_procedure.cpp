...
int decision_procedure::evaluate(std::vector<pdrh::mode *> path)
{
    ...
    f_stream << raw_filename << "_" << global_config.reach_depth_max << "_0.smt2";
    ...
    else if(first_res == 1)
    {
        ...
        {
            std::cout << "Problem occured while removing one of auxilary files" << std::endl;
            return decision_procedure::ERROR;
        }
    }
    else
    {
        ...
        f_stream.str("");
        f_stream << raw_filename << "_" << global_config.reach_depth_max << "_0.c.smt2";
        std::string smt_c_filename = f_stream.str();
        // writing to the file
        std::ofstream smt_c_file;
        smt_c_file.open(smt_c_filename.c_str());
        smt_c_file << pdrh::reach_c_to_smt2(path);
        smt_c_file.close();
        // calling dreal here
