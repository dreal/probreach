...
void DebugLogMatrix::printMatrix(char* desc, int id,
                                 std::shared_ptr<std::vector<double> > arg) {
  //#ifdef _DEBUG
  ofstream out;
...
void DebugLogMatrix::printMatrix(
    char* desc, int id,
    std::shared_ptr<std::vector<std::vector<double> > > arg) {
  //#ifdef _DEBUG
  ofstream out;
  out.open("log.txt", ios_base::app);
  out << string(desc) << " " << id << endl << "------" << endl;
  ...
  out << "rows=" << rows << " cols=" << cols << " length=" << rows * cols
      << endl;
  for (size_t i = 0; i < rows; i++) {
    for (size_t j = 0; j < cols; j++) {
      out << "arg[" << i * cols + j << "] = arg[" << i << "][" << j
          << "]=" << (*arg)[i][j] << endl;
    }
  }
  out << endl;
...
void DebugLogMatrix::printMatrix(char* desc, int id, gsl_matrix* arg) {
  ...
  for (size_t j = 0; j < cols; j++) {
    for (size_t i = 0; i < rows; i++) {
      out << "arg[" << j * rows + i << "] = arg[" << i << "][" << j
          << "]=" << gsl_matrix_get(arg, i, j) << endl;
    }
  }
  out << endl;
  //#endif
