...
DoubleMatrix::DoubleMatrix() { matrix = gsl_matrix_calloc(1, 1); }
...
DoubleMatrix::~DoubleMatrix(void) { clear(); }
...
std::shared_ptr<std::vector<std::vector<double> > >& DoubleMatrix::getPtrMatrix(
    void) {
  ptr_vector_matrix =
      std::make_shared<std::vector<std::vector<double> > >(matrix->size1);
  for (size_t i = 0; i < matrix->size1; ++i) {
    (*ptr_vector_matrix)[i].resize(matrix->size2);
    const double* begin_ptr = gsl_matrix_const_ptr(matrix, i, 0);
    const double* end_ptr =
        gsl_matrix_const_ptr(matrix, i, matrix->size2 - 1) + 1;
    if (begin_ptr != NULL) {
...
std::vector<double>& DoubleMatrix::getData(void) {
  ...
  for (size_t r = 0; r < matrix->size1; ++r) {
    std::copy(matrix->data + row_start,
              matrix->data + row_start + matrix->size2,
              data.begin() + dest_pos);
...
void DoubleMatrix::copy(DoubleMatrix* arg) {
  if (matrix->size1 != arg->matrix->size1 ||
      matrix->size2 != arg->matrix->size2) {
    clear();
...
DoubleMatrix* DoubleMatrix::mmul(DoubleMatrix* other) {
  ...
  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, matrix, other->matrix, 0.0,
                 C->matrix);
  return C;
}
