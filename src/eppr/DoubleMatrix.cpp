...
DoubleMatrix::getPtrMatrix(void)
{
  ptr_vector_matrix =
...
{
  if (
    matrix->size1 != arg->matrix->size1 || matrix->size2 != arg->matrix->size2)
  {
    clear();
...
DoubleMatrix *DoubleMatrix::mmul(DoubleMatrix *other)
{
  ...
  gsl_blas_dgemm(
    CblasNoTrans, CblasNoTrans, 1.0, matrix, other->matrix, 0.0, C->matrix);
  return C;
}
