...
void GPEP::doTraining(void)
{
  ...
  invC = algebra->solvePositive(
    gauss->C->add(Sigma_tilde), algebra->createEye(mu_tilde->getLength()));
}
...
std::shared_ptr<Gauss> GPEP::expectationPropagation(double tolerance)
{
  ...
  while ((std::abs(logZ - logZold) > tol) & (steps < MaxIter))
  {
    ...
    // Term, eps_damp);
    std::shared_ptr<EPupdate> update = ep_update(
      cavGauss,
      Term,
      eps_damp,
      NULL,
...
double GPEP::computeMarginalMoments(
  ...
  std::shared_ptr<IMatrix> Term)
{
  ...
  std::shared_ptr<IMatrix> A =
    gauss->LC_t->mmul(tmp)->add(algebra->createEye(N)->mul(CORRECTION_FACTOR));
...
void GPEP::gausshermite(
  int n,
  std::shared_ptr<IMatrix> x,
...
std::shared_ptr<CavGauss> GPEP::ComputeCavities(
  std::shared_ptr<Gauss> gauss,
  std::shared_ptr<IMatrix> Term)
{
...
std::shared_ptr<EPupdate> GPEP::ep_update(
  std::shared_ptr<CavGauss> cavGauss,
  std::shared_ptr<IMatrix> Term,
  double eps_damp,
  void *LogLikFunc,
...
std::shared_ptr<IMatrix> GPEP::GaussHermiteNQ(
  std::shared_ptr<IMatrix> FuncPar_p,
  std::shared_ptr<IMatrix> FuncPar_q,
  std::shared_ptr<IMatrix> m,
  std::shared_ptr<IMatrix> v,
  std::shared_ptr<IMatrix> xGH,
  std::shared_ptr<IMatrix> logwGH,
  std::shared_ptr<IMatrix> Cumul)
{
...
std::shared_ptr<IMatrix> GPEP::logprobitpow(
  std::shared_ptr<IMatrix> X,
  std::shared_ptr<IMatrix> LikPar_p,
  std::shared_ptr<IMatrix> LikPar_q)
{
...
double GPEP::ncdflogbc(double x)
{
  ...
  return -0.5 * log(M_PI) - log_2 - 0.5 * z * z - log(z) +
         log(
           1 - 1 / z + 3 / pow(z, 4) - 15 / pow(z, 6) + 105 / pow(z, 8) -
           945 / pow(z, 10));
}

std::shared_ptr<ProbitRegressionPosterior>
GPEP::getGpPosterior(std::shared_ptr<GpDataset> testSet)
{
