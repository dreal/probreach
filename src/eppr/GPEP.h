...
class GPEP : public AbstractGP {
 ...
 private:
  double computeMarginalMoments(std::shared_ptr<Gauss> gauss,
                                std::shared_ptr<IMatrix> Term);
  void gausshermite(int n, std::shared_ptr<IMatrix> x,
                    std::shared_ptr<IMatrix> w);
  std::shared_ptr<CavGauss> ComputeCavities(std::shared_ptr<Gauss> gauss,
                                            std::shared_ptr<IMatrix> Term);
  std::shared_ptr<EPupdate> ep_update(std::shared_ptr<CavGauss> cavGauss,
                                      std::shared_ptr<IMatrix> Term,
  ...
  std::shared_ptr<IMatrix> GaussHermiteNQ(std::shared_ptr<IMatrix> FuncPar_p,
  ...
  std::shared_ptr<IMatrix> logprobitpow(std::shared_ptr<IMatrix> X,
                                        std::shared_ptr<IMatrix> LikPar_p,
                                        std::shared_ptr<IMatrix> LikPar_q);
  static double ncdflogbc(double x);

 public:
  std::shared_ptr<ProbitRegressionPosterior> getGpPosterior(
