...
private:
  double computeMarginalMoments(
    std::shared_ptr<Gauss> gauss,
    std::shared_ptr<IMatrix> Term);
  void
  gausshermite(int n, std::shared_ptr<IMatrix> x, std::shared_ptr<IMatrix> w);
  std::shared_ptr<CavGauss>
  ComputeCavities(std::shared_ptr<Gauss> gauss, std::shared_ptr<IMatrix> Term);
  std::shared_ptr<EPupdate> ep_update(
  ...
  std::shared_ptr<IMatrix> GaussHermiteNQ(
  ...
  std::shared_ptr<IMatrix> logprobitpow(
...
public:
  ...
  getGpPosterior(std::shared_ptr<GpDataset> testSet);
