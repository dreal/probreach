...
std::shared_ptr<IMatrix> GSLAlgebra::createMatrix(
...
std::shared_ptr<IMatrix> GSLAlgebra::invertPositive(
    std::shared_ptr<IMatrix> arg) {
  return solvePositive(arg, createEye(arg->getRows()));
  ;
}

std::shared_ptr<IMatrix> GSLAlgebra::solve(std::shared_ptr<IMatrix> A,
...
std::shared_ptr<IMatrix> GSLAlgebra::solvePositive(std::shared_ptr<IMatrix> A,
                                                   std::shared_ptr<IMatrix> B) {
  MatrixGSL* A_ = dynamic_cast<MatrixGSL*>(A.get());
  MatrixGSL* B_ = dynamic_cast<MatrixGSL*>(B.get());
  DoubleMatrix* X = DoubleMatrix::solve(A_->getMatrixPtr(), B_->getMatrixPtr());
...
void GSLAlgebra::solvePositiveInPlace(std::shared_ptr<IMatrix> A,
                                      std::shared_ptr<IMatrix> B) {
  MatrixGSL* A_ = dynamic_cast<MatrixGSL*>(A.get());
  MatrixGSL* B_ = dynamic_cast<MatrixGSL*>(B.get());
  DoubleMatrix* X = DoubleMatrix::solve(A_->getMatrixPtr(), B_->getMatrixPtr());
 ...
 */
std::vector<std::shared_ptr<IMatrix>>& GSLAlgebra::svd(
    std::shared_ptr<IMatrix> arg) {
