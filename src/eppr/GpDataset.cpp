...
GpDataset::GpDataset(int dimension) { init(dimension, 0); }
...
void GpDataset::init(int dimension, int preemptedSize) {
  ...
  std::shared_ptr<std::vector<std::vector<double>>> x(
      new std::vector<std::vector<double>>(preemptedSize));
...
void GpDataset::add(std::vector<std::vector<double>>& x2,
                    std::vector<double>& y2, std::vector<double>& noise2) {
  ...
  if (size12 > getPreemptedSize()) {
    std::shared_ptr<std::vector<std::vector<double>>> x12(
        new std::vector<std::vector<double>>(size12));
    for (int i = 0; i < size12; ++i) (*x12)[i].resize(dimension);
    std::shared_ptr<std::vector<double>> y12(new std::vector<double>(size12));
    std::shared_ptr<std::vector<double>> noise12(
        new std::vector<double>(size12));
...
int GpDataset::getSize(void) { return size; }
...
int GpDataset::getDimension(void) { return dimension; }

std::shared_ptr<std::vector<std::vector<double>>>& GpDataset::getInstance(
    void) {
  return dataX;
}

std::vector<double>& GpDataset::getInstance(int i) { return (*dataX)[i]; }
...
double GpDataset::getTargetMean(void) {
  double sum = 0;
  for (int i = 0; i < getSize(); i++) sum += (*dataY)[i];
  return sum / getSize();
}
...
std::vector<double>& GpDataset::getZeroMeanTargets(void) {
  ...
  for (int i = 0; i < getSize(); i++) zeroMeanY[i] -= meanY;
  return zeroMeanY;
}

std::shared_ptr<std::vector<std::vector<double>>>
GpDataset::calculateCovariances(std::shared_ptr<KernelFunction> func) {
  ...
  std::shared_ptr<std::vector<std::vector<double>>> result =
      allocateGramMatrix(n);
...
bool GpDataset::canUsedCachedResults(std::shared_ptr<KernelFunction> func) {
  ...
  for (it_l = cachedHyperparams.begin(), it_r = r.begin();
       it_l != cachedHyperparams.end(); it_l++, it_r++) {
    if ((*it_l) != (*it_r)) {
      equal = false;
      break;
    }
...
std::shared_ptr<std::vector<std::vector<double>>> GpDataset::allocateGramMatrix(
    int n) {
  if (cachedGramMatrix != nullptr && cachedGramMatrix->size() == (size_t)n &&
      (*cachedGramMatrix)[0].size() == (size_t)n)
    return cachedGramMatrix;
  else {
    std::shared_ptr<std::vector<std::vector<double>>> pt =
        std::make_shared<std::vector<std::vector<double>>>(n);
    for (size_t i = 0; i < pt->size(); ++i) (*pt)[i].resize(n);
    return pt;
  }
}

std::shared_ptr<std::vector<double>> GpDataset::calculateVariances(
    std::shared_ptr<KernelFunction> func) {
  int n = this->getSize();
  std::shared_ptr<std::vector<std::vector<double>>> x = this->dataX;
...
GpDataset::calculateCovariances(std::shared_ptr<KernelFunction> func,
  ...
  std::shared_ptr<std::vector<std::vector<double>>> result =
      std::make_shared<std::vector<std::vector<double>>>(n1);
...
bool GpDataset::isModified() { return changedSinceGramCalculation; }
