...
bool GpDataset::canUsedCachedResults(std::shared_ptr<KernelFunction> func)
{
  ...
  if (cachedHyperparams.size() != r.size())
    return false;

  for (it_l = cachedHyperparams.begin(), it_r = r.begin();
       it_l != cachedHyperparams.end();
       it_l++, it_r++)
  {
    if ((*it_l) != (*it_r))
    {
...
std::shared_ptr<std::vector<std::vector<double>>>
GpDataset::allocateGramMatrix(int n)
{
  if (
    cachedGramMatrix != nullptr && cachedGramMatrix->size() == (size_t)n &&
...
GpDataset::calculateVariances(std::shared_ptr<KernelFunction> func)
{
  int n = this->getSize();
  std::shared_ptr<std::vector<std::vector<double>>> x = this->dataX;
...
std::shared_ptr<std::vector<std::vector<double>>>
GpDataset::calculateCovariances(
