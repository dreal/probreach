...
HyperparamLogLikelihood::HyperparamLogLikelihood(std::shared_ptr<AbstractGP> gp,
                                                 bool logspace) {
  this->gp = gp;
...
double HyperparamLogLikelihood::getValueAt(
    std::shared_ptr<std::vector<double> > point) {
  std::shared_ptr<std::vector<double> > vec =
      std::make_shared<std::vector<double> >(point->size());
  std::copy(point->begin(), point->end(), vec->begin());

  if (logspace)
    for (size_t i = 0; i < vec->size(); i++) (*vec)[i] = exp((*vec)[i]);
...
std::shared_ptr<std::vector<double> > HyperparamLogLikelihood::getGradientAt(
    std::shared_ptr<std::vector<double> > point) {
  std::shared_ptr<std::vector<double> > vec =
      std::make_shared<std::vector<double> >(point->size());
  std::copy(point->begin(), point->end(), vec->begin());
  ...
  try {
    std::shared_ptr<std::vector<double> > grad =
        gp->getMarginalLikelihoodGradient();
    return grad;
  } catch (const char e) {
    throw(e);
  }
