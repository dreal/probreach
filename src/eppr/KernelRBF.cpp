...
double KernelRBF::calculateDerivative(std::vector<double>& x1,
                                      std::vector<double>& x2, int i) {
  return -invLengthscale2 * (x1[i] - x2[i]) * calculate(x1, x2);
}

double KernelRBF::calculateSecondDerivative(std::vector<double>& x1,
                                            std::vector<double>& x2, int i,
                                            int j) {
  ...
  double k = k0 * ki * kj;
  if (i == j) k = k - 2 * invLengthscale2 * k0;
  return k;
}

double KernelRBF::calculateHyperparamDerivative(std::vector<double>& x1,
...
std::vector<double>& KernelRBF::getHyperparameters(void) { return hyp; }

std::vector<double>& KernelRBF::getDefaultHyperarameters(
  ...
  for (int d = 0; d < dim; d++) {
    ...
    min = -std::numeric_limits<double>::infinity();
    for (int i = 0; i < n; i++) {
      const std::shared_ptr<std::vector<std::vector<double> > >& xcur =
          data->getInstance();
