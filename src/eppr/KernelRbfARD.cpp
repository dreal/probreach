...
void KernelRbfARD::setHyperarameters(std::vector<double>& hyp) {
  ...
  this->amplitude2 = a * a;
  this->invLengthscale2 =
      std::make_shared<std::vector<double> >(this->hyp->size() - 1);
...
double KernelRbfARD::calculate(std::vector<double>& x1,
...
double KernelRbfARD::calculateDerivative(std::vector<double>& x1,
                                         std::vector<double>& x2, int i) {
  return -(*invLengthscale2)[i] * (x1[i] - x2[i]) * calculate(x1, x2);
}
double KernelRbfARD::calculateSecondDerivative(std::vector<double>& x1,
                                               std::vector<double>& x2, int i,
                                               int j) {
  ...
  double k = k0 * ki * kj;
  if (i == j) k = k - 2 * (*invLengthscale2)[i] * k0;

  return k;
}
double KernelRbfARD::calculateHyperparamDerivative(std::vector<double>& x1,
                                                   std::vector<double>& x2,
                                                   int hyperparamIndex) {
...
std::vector<double>& KernelRbfARD::getDefaultHyperarameters(
  ...
  if ((*defaultHyp)[0] == 0) (*defaultHyp)[0] = 1;
