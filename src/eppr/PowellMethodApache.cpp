...
// std::shared_ptr<PointValue>
// PowellMethodApache::optimise(std::shared_ptr<ObjectiveFunction> func,
// std::shared_ptr<std::vector<double> > init)
//{
//	//gsl_ieee_env_setup();
//
...
//newPointAndDirection(x, d, alphaMin); 			x = result[0];
//
//			if ((fX2 - fVal) > delta) {
//				delta = fX2 - fVal;
//				bigInd = i;
...
//		if (stop || iter > 1000) {
//				result = (fVal > fX) ?
//std::make_shared<PointValue>(current) :
//std::make_shared<PointValue>(previous); 				return result;
//		}
...
//
//		x1 = x;
//		fX2 = fn.func->getValueAt(std::make_shared<std::vector<double>
//>(x2));
//
//		if (fX > fX2) {
...
std::vector<std::vector<double> >& PowellMethodApache::newPointAndDirection(
    std::vector<double>& p, std::vector<double>& d, double optimum) {
...
PointValue& PowellMethodApache::search(std::vector<double>& startPoint,
...
bool PowellMethodApache::converged(int iteration, PointValue& previous,
                                   PointValue& current) {
  return false;
}
