...
class PowellMethodApache : public LocalOptimisation {
 public:
  ...
  // virtual std::shared_ptr<PointValue>
  // optimise(std::shared_ptr<ObjectiveFunction> func,
  //	std::shared_ptr<std::vector<double> > init);
  virtual std::shared_ptr<PointValue> optimise(
  ...
  std::vector<std::vector<double> >& newPointAndDirection(
      std::vector<double>& p, std::vector<double>& d, double optimum);
  PointValue srch;
  PointValue& search(std::vector<double>& startPoint,
