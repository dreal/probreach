...
ProbitRegressionPosterior::getClassProbabilities(void) {
  return probabilities;
}

std::shared_ptr<std::vector<double>> ProbitRegressionPosterior::getLowerBound(
    double beta) {
  std::shared_ptr<std::vector<double>> bounds =
      std::make_shared<std::vector<double>>(probabilities->size());
  for (size_t i = 0; i < bounds->size(); i++)
    (*bounds)[i] = standardNormalCDF(
        ((*getMean())[i] - beta * (*getStandardDeviation())[i]));
  //* (*cached_denominator)[i]);
  return bounds;
}

std::shared_ptr<std::vector<double>> ProbitRegressionPosterior::getUpperBound(
    double beta) {
  std::shared_ptr<std::vector<double>> bounds =
      std::make_shared<std::vector<double>>(probabilities->size());
  for (size_t i = 0; i < bounds->size(); i++)
