...
std::vector<std::vector<double>>& RegularSampler::sample(
    int n, std::vector<std::shared_ptr<Parameter>>& params) {
  ...
  for (int d = dim - 1; d >= 0; d--) {
    const std::vector<double>& lin = linspace(
        pointsPerDim, params[d]->getLowerBound(), params[d]->getUpperBound());
