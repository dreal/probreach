...
std::shared_ptr<GpDataset> SmoothedModelCheker::performStatisticalModelChecking(
...
SmoothedModelCheker::performSmoothedModelChecking(
    std::shared_ptr<GpDataset> data,
    std::vector<std::shared_ptr<Parameter> >& parameters,
    std::shared_ptr<SmmcOptions> options,
    std::vector<std::vector<double> >& paramValueSet) {
  std::shared_ptr<AnalyticApproximation> approx =
      getAnalyticApproximation(data, parameters, options);

  std::shared_ptr<GpDataset> testSet =
      std::make_shared<GpDataset>(paramValueSet);
  sp_smoothModel = approx->getValuesAt(testSet);
  return sp_smoothModel;
}

std::shared_ptr<AnalyticApproximation>
SmoothedModelCheker::getAnalyticApproximation(
    std::shared_ptr<GpDataset> data,
    std::vector<std::shared_ptr<Parameter> >& parameters,
    std::shared_ptr<SmmcOptions> options) {
  ...
  if (options->getHyperparamOptimisation()) {
    ...
    std::vector<double>& hyp =
        gp->getKernel()->getDefaultHyperarameters(gp->getTrainingSet());
  ...
  //		for (int i = 1; i < dim; i++)
  //			System.out.println("lengthscale: "
  //				+ options.getKernelGP().getHypeparameters()[i +
  //1]);
  //	}
  //}
...
void SmoothedModelCheker::optimiseGPHyperParameters(
    std::shared_ptr<GPEP> gp, std::shared_ptr<SmmcOptions> options) {
  bool logspace = true;
  std::shared_ptr<HyperparamLogLikelihood> func =
      std::make_shared<HyperparamLogLikelihood>(gp, logspace);
  std::shared_ptr<GpDataset> train = gp->getTrainingSet();
  std::shared_ptr<std::vector<double> > init =
      std::make_shared<std::vector<double> >(
          gp->getKernel()->getDefaultHyperarameters(train));
  if (variance(std::make_shared<std::vector<double> >(train->getTargets())) ==
      0) {
  ...
  if (logspace)
    for (size_t i = 0; i < init->size(); i++) (*init)[i] = log((*init)[i]);

  std::shared_ptr<LocalOptimisation> alg =
      std::make_shared<PowellMethodApache>();
  std::shared_ptr<PointValue> best = alg->optimise(func, init);
  for (int r = 0; r < options->getHyperparamOptimisationRestarts(); r++) {
    std::shared_ptr<std::vector<double> > currentInit =
        std::make_shared<std::vector<double> >(init->size());
    ...
    if (curr->getValue() > best->getValue()) best = curr;
    //	printProgress();
  }
  // resetProgress("\n");
...
double SmoothedModelCheker::variance(
    std::shared_ptr<std::vector<double> > vec) {
  double mean = 0.0;
  std::vector<double>::iterator v;
  for (v = vec->begin(); v != vec->end(); ++v) mean += (*v);
