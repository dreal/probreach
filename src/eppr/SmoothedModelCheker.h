...
class SmoothedModelCheker {
 ...
 public:
  ...
  virtual std::shared_ptr<GpDataset> performStatisticalModelChecking(
 ...
 public:
  std::shared_ptr<ProbitRegressionPosterior> performSmoothedModelChecking(
      std::shared_ptr<GpDataset> data,
      std::vector<std::shared_ptr<Parameter> >& parameters,
      std::shared_ptr<SmmcOptions> options,
      std::vector<std::vector<double> >& paramValueSet);
  std::shared_ptr<AnalyticApproximation> getAnalyticApproximation(
      std::shared_ptr<GpDataset> data,
      std::vector<std::shared_ptr<Parameter> >& parameters,
      std::shared_ptr<SmmcOptions> options);
  void optimiseGPHyperParameters(std::shared_ptr<GPEP> gp,
                                 std::shared_ptr<SmmcOptions> options);
