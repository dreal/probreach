...
int main(int argc, char *argv[])
{
  ...
  gsl_qrng *m = gsl_qrng_alloc(
    gsl_qrng_sobol, static_cast<unsigned int>(pdrh::par_map.size()));
...
#pragma omp critical
      ...
      int res = decision_procedure::evaluate(
        paths, {GPicdf_sample, mu_sample}, dreal::solver_bin, "");
...
#pragma omp critical
  ...
  for (int l = 0; l < NondetP; l++)
  {
    ...
    {
      LOarray[l] = 0;
      UParray[l] = gsl_cdf_beta_Pinv(
        1 - ((1 - conf) / 2), Sats[l] + 1, Total_samples - Sats[l]);
    }
    else if (Sats[l] == Total_samples)
    {
      LOarray[l] =
        gsl_cdf_beta_Pinv((1 - conf) / 2, Sats[l], Total_samples - Sats[l] + 1);
      UParray[l] = 1;
    }
    else
    {
  ...
  DebugLogMatrix::printMatrix(
    "getClassProbabilities", __LINE__, result->getClassProbabilities());
  DebugLogMatrix::printMatrix(
    "getLowerBound(beta)", __LINE__, result->getLowerBound(beta));
  DebugLogMatrix::printMatrix(
    "getUpperBound(beta)", __LINE__, result->getUpperBound(beta));
  //--------------GP----------------
