...
// Unix
#if (                                                                          \
  (ELPP_OS_LINUX || ELPP_OS_MAC || ELPP_OS_FREEBSD || ELPP_OS_SOLARIS) &&      \
  (!ELPP_OS_WINDOWS))
...
#if (                                                                          \
  !defined(ELPP_DISABLE_DEBUG_LOGS) && (ELPP_LOGGING_ENABLED) &&               \
...
public:
  ...
  static Level convertFromString(const char *levelStr)
  {
    ...
    if ((strcmp(levelStr, "INFO") == 0) || (strcmp(levelStr, "info") == 0))
      return Level::Info;
    if (
      (strcmp(levelStr, "WARNING") == 0) || (strcmp(levelStr, "warning") == 0))
    ...
    if ((strcmp(levelStr, "FATAL") == 0) || (strcmp(levelStr, "fatal") == 0))
      return Level::Fatal;
    if (
      (strcmp(levelStr, "VERBOSE") == 0) || (strcmp(levelStr, "verbose") == 0))
  ...
  static inline void forEachLevel(
...
public:
  ...
  static ConfigurationType convertFromString(const char *configStr)
  {
    ...
    if (
      (strcmp(configStr, "FORMAT") == 0) || (strcmp(configStr, "format") == 0))
...
public:
  ...
  static inline char *convertAndAddToBuff(
    std::size_t n,
    int len,
    char *buf,
    const char *bufLim,
...
public:
  ...
  public:
    ...
    {
      return (
        (conf != nullptr) && (conf->level() == m_level) &&
...
class Configurations
  : public base::utils::
      RegistryWithPred<Configuration, Configuration::Predicate>
{
public:
  ...
  bool hasConfiguration(ConfigurationType configurationType)
  {
    ...
    LevelHelper::forEachLevel(
      ...
      {
        if (hasConfiguration(
              LevelHelper::castFromInt(lIndex), configurationType))
        {
          result = true;
        }
        return result;
...
public:
  ...
  inline base::type::fstream_t *fileStream(Level level)
  {
    return getConfigByRef<base::FileStreamPtr>(
             level, &m_fileStreamMap, "fileStream")
      .get();
  }
...
private:
  ...
  template <typename Conf_T>
  inline Conf_T unsafeGetConfigByVal(
    Level level,
    const std::map<Level, Conf_T> *confMap,
    const char *confName)
  {
  ...
  template <typename Conf_T>
  inline Conf_T &unsafeGetConfigByRef(
    Level level,
    std::map<Level, Conf_T> *confMap,
    const char *confName)
  {
  ...
  void build(Configurations *configurations)
  {
         ...
         ++it)
    {
      ...
      else if (conf->configurationType() == ConfigurationType::Format)
      {
        ...
        conf->configurationType() == ConfigurationType::MillisecondsWidth)
      {
        setValue(
          Level::Global,
        ...
        conf->configurationType() == ConfigurationType::PerformanceTracking)
      {
        setValue(
      ...
      else if (conf->configurationType() == ConfigurationType::MaxLogFileSize)
      {
        ...
        conf->configurationType() == ConfigurationType::LogFlushThreshold)
      {
        setValue(
          conf->level(),
          static_cast<std::size_t>(getULong(conf->value())),
  ...
  unsigned long getULong(std::string confVal)
  {
    ...
    valid = !confVal.empty() && std::find_if(
                                  confVal.begin(),
                                  confVal.end(),
                                  [](char c) {
  ...
  void insertFile(Level level, const std::string &fullFilename)
  {
    ...
    auto create = [&](Level level)
    {
      ...
      if (fs == nullptr)
      {
        ...
        ELPP_INTERNAL_ERROR(
          "Setting [TO_FILE] of [" << LevelHelper::convertToString(level)
                                   << "] to FALSE",
          false);
    ...
    create(
      m_filenameMap.empty() && m_fileStreamMap.empty() ? Level::Global : level);
  }

  bool unsafeValidateFileRolling(
    Level level,
...
class RegisteredHitCounters
  : public base::utils::
      RegistryWithPred<base::HitCounter, base::HitCounter::Predicate>
{
public:
...
public:
  ...
  void setFromArgs(const base::utils::CommandLineArgs *commandLineArgs)
  {
    ...
    else if (commandLineArgs->hasParamWithValue("--V"))
    {
      ...
      (commandLineArgs->hasParamWithValue("-vmodule")) && vModulesEnabled())
    {
...
public:
  ...
  inline bool uninstallCustomFormatSpecifier(const char *formatSpecifier)
  {
    ...
    std::vector<CustomFormatSpecifier>::iterator it = std::find(
      ...
      formatSpecifier);
    if (
      it != m_customFormatSpecifiers.end() &&
...
private:
  ...
  inline bool
  installCallback(const std::string &id, std::map<std::string, TPtr> *mapT)
  {
  ...
  template <typename T, typename TPtr>
  inline void
  uninstallCallback(const std::string &id, std::map<std::string, TPtr> *mapT)
  {
    if (mapT->find(id) != mapT->end())
    {
      mapT->erase(id);
    ...
    if (m_data->dispatchAction() == base::DispatchAction::NormalLog)
    {
            ...
            m_data->logMessage()->level()))
      {
        ...
        if (fs != nullptr)
        {
          ...
          {
            if (
              ELPP->hasFlag(LoggingFlag::ImmediateFlush) ||
        ...
        else
        {
          ELPP_INTERNAL_ERROR(
            "Log file for ["
              ...
              << "] "
              << "has not been configured but [TO_FILE] is configured to TRUE. "
                 "[Logger ID: "
...
public:
  ...
  build(const LogMessage *logMessage, bool appendNewLine) const
  {
    ...
    if (
      logMessage->level() == Level::Verbose &&
...
#define ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG(temp)                             \
  ...
  inline MessageBuilder &operator<<(const QMap<K, V> &map_)
  {
    ...
    for (int index_ = 0; begin != end && index_ < max_; ++index_, ++begin)
    {
      ...
      m_logger->stream() << ELPP_LITERAL(")");
      m_logger->stream()
        << ((index_ < keys.size() - 1) ? m_containerLogSeperator
                                       : ELPP_LITERAL(""));
    }
    if (begin != end)
    {
...
#undef ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG
  ...
  writeIterator(Iterator begin_, Iterator end_, std::size_t size_)
  {
         ...
         ++i, ++begin_)
    {
      operator<<(*begin_);
      m_logger->stream()
        << ((i < size_ - 1) ? m_containerLogSeperator : ELPP_LITERAL(""));
    }
    if (begin_ != end_)
    {
      m_logger->stream() << ELPP_LITERAL("...");
...
protected:
    ...
    bool needLock = true)
  {
    ...
    else
    {
      ...
      {
        m_logger
          ->acquireLock(); // This should not be unlocked by checking m_proceed because
  ...
  const Args &...args)
{
  ...
  ELPP_INTERNAL_ERROR(
    "Too many arguments provided. Unable to handle. Please provide more format "
    "specifiers",
    false);
}
template <typename T>
...
private:
  ...
  const char *m_func;
  inline void init(
    base::PerformanceTracker *performanceTracker,
...
public:
    ...
    const char *func = "")
  {
    ...
    if (m_enabled)
    {
      ...
      PerformanceTrackingCallback *callback = nullptr;
      for (const std::
             pair<std::string, base::type::PerformanceTrackingCallbackPtr> &h :
           ELPP->m_performanceTrackingCallbacks)
      {
        callback = h.second.get();
        if (callback != nullptr && callback->enabled())
...
protected:
  void handle(const PerformanceTrackingData *data)
  {
    ...
    el::base::Writer(
      m_data->performanceTracker()->level(),
      m_data->file(),
      m_data->line(),
      m_data->func())
  ...
  void generateNew(void)
  {
    ...
    { // Skip StackTrace c'tor and generateNew
      for (std::size_t i = kStackStart; i < size; ++i)
      {
        ...
        if (
          mangName != nullptr && hex != nullptr && addr != nullptr &&
          mangName < hex)
        {
          *mangName++ = '\0';
          *hex++ = '\0';
...
public:
  ...
  static void configureFromGlobal(const char *globalConfigurationFilePath)
  {
    ...
    auto configure = [&](void)
    {
      ELPP_INTERNAL_INFO(
        8,
        "Configuring logger: '" << logger->id() << "' with configurations \n"
                                << ss.str() << "\n--------------");
      Configurations c;
...
{
  CLOG_IF(ptr == nullptr, FATAL, loggers)
    << "Check failed: [" << name << " != nullptr]";
  return ptr;
}
} // namespace utils
} // namespace base
