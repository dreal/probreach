...
void parse_pdrh(string filename) {
  ...
  //    if(res != 0)
  //    {
  //        CLOG(ERROR, "parser") << "Problem occured while preprocessing " <<
...
int main(int argc, char* argv[]) {
  ...
  //    CLOG_IF(global_config.verbose_result, INFO, "parser") << "Model type: "
  //    << pdrh::model_type; if(global_config.solver_type ==
  //    solver::type::DREAL)
  //    {
  //        CLOG_IF(global_config.verbose_result, INFO, "parser") << "Primary
  //        solver: dReal";
  //    }
  ...
  //    {
  //        CLOG_IF(global_config.verbose_result, INFO, "parser") << "Secondary
  //        solver: not defined";
  //    }
  //    else
  //    {
  ...
  //        {
  //            CLOG_IF(global_config.verbose_result, INFO, "parser") <<
  //            "Secondary solver: dReal";
  //        }
  //        else if(global_config.secondary_solver_type == solver::type::ISAT)
  //        {
  //            CLOG_IF(global_config.verbose_result, INFO, "parser") <<
  //            "Secondary solver: iSAT";
  //        }
  //        else if(global_config.secondary_solver_type ==
  //        solver::type::UNKNOWN_SOLVER)
  //        {
  ...
  switch (pdrh::model_type) {
    ...
    case pdrh::HA: {
      //            int res =
      //            algorithm::evaluate_ha(global_config.reach_depth_min,
      //            global_config.reach_depth_max); if (res ==
      //            decision_procedure::SAT)
      //            {
      ...
      //            }
      //            break;
      // cout << "Simulating a path: " <<
      // ap::simulate_path(ap::get_all_paths({}).front(), ap::init_to_box({}),
      // {}) << endl; cout << global_config.global_time << endl; cout <<
      // global_config.sample_time << endl; global_config.ode_discretisation =
      // 1; cout << "Noise variance: " << global_config.noise_var << endl; cout
      // << "Simulation: " << ap::simulate({}) << endl;
    ...
    case pdrh::PHA: {
      ...
      if (global_config.chernoff_flag) {
        probability = algorithm::evaluate_pha_chernoff(
        ...
        probability = algorithm::evaluate_pha_bayesian(
            global_config.reach_depth_min, global_config.reach_depth_max,
            global_config.bayesian_acc, global_config.bayesian_conf);
      } else {
        probability = algorithm::evaluate_pha(global_config.reach_depth_min,
      ...
      std::cout << scientific << probability << " | "
                << capd::intervals::width(probability) << std::endl;
    ...
    case pdrh::NPHA: {
      if (global_config.sobol_flag) {
        pair<box, capd::interval> probability = algorithm::evaluate_npha_sobol(
            global_config.reach_depth_min, global_config.reach_depth_max,
        ...
        std::map<box, capd::interval> probability_map =
            algorithm::evaluate_npha(global_config.reach_depth_min,
                                     global_config.reach_depth_max);
        for (auto it = probability_map.cbegin(); it != probability_map.cend();
             it++) {
          std::cout << scientific << it->first << " | " << it->second
                    << std::endl;
        }
      }
      break;
    ...
    // parameter synthesis
    case pdrh::PSY: {
      // CLOG(ERROR, "algorithm") << "Parameter synthesis is currently not
      // supported"; exit(EXIT_FAILURE);
      ...
      map<string, vector<pair<pdrh::node*, pdrh::node*>>> time_series =
          csvparser::parse(global_config.series_filename);
      tuple<vector<box>, vector<box>, vector<box>> boxes =
          algorithm::evaluate_psy(time_series);
    ...
    case pdrh::NHA: {
      CLOG(ERROR, "algorithm")
          << "Nondeterministic hybrid automata is currently not supported";
      exit(EXIT_FAILURE);
      // break;
    }
  }
  ...
  //
  //    capd::interval ref_interval = sqrt((Sx*g) /
  //    (2*v0*v0*cos(alpha)*sin(alpha))-1); map<string, capd::interval> ref_map;
  //    ref_map.insert(make_pair("K", ref_interval));
  //    box ref_box(ref_map);
  //    cout << "Ref box:" << endl;
  //    cout << setprecision(16) << ref_box << endl;
  //
  //    capd::interval lower_bound = sqrt( (Sx * g) / (sin(2 * alpha) * (0.7*0.7
  //    + 1)) ); cout << setprecision(16) << lower_bound << endl;
