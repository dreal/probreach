...
map<box, capd::interval> outputparser::parse_p_map(string filename)
{
    ...
    if(file.is_open())
    {
        ...
        while(getline(file, line))
        {
            ...
            if(pos == string::npos)
            {
                cerr << "Could not locate probability enclosure in line: " << line << endl;
                exit(EXIT_FAILURE);
            }
            ...
            res_map.insert(make_pair(box(box_string), capd::interval(p_string.substr(1, pos - 1), p_string.substr(pos + 1, p_string.length() - pos - 2))));
        }
    }
    else
    {
