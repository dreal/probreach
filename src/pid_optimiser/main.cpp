...
//    cout << pdrh::model_to_string() << endl;
    ...
    for(string param : param_names)
    {
        ...
        while(capd::intervals::width(conf_intersection) < acc)
        {
            ...
            if(capd::intervals::width(conf_intersection) < acc)
            {
                num_points *= 2;
            }
            // updating the result
            ...
            if(capd::intervals::width(conf_intersection) < acc && (conf_intersection.rightBound() >= 1 || conf_intersection.leftBound() <= 0))
            {
                break;
            }
        }
