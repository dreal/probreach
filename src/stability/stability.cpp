...
bool stability::jury_test(std::vector<double> poly) {
  ...
  for (size_t j = 0; j < n - 2; j++) {
    ...
    for (size_t i = 0; i < m; i++) {
      new_row.push_back(poly.at(m) * poly.at(i + 1) -
                        poly.at(0) * poly.at(m - 1 - i));
    }
...
bool stability::is_stable(std::map<std::string, pdrh::node *> odes, double T,
                          box init, box param) {
  // return true;
  vector<string> vars;
  ...
  double D[global_config.controller.controller_output.size()]
          [global_config.controller.plant_output.size()];
  ...
  //    global_config.controller.controller_output << endl; cout << "Plant
  //    output: " << global_config.controller.plant_output << endl;
  ...
  for (size_t i = 0; i < global_config.controller.controller_output.size();
       i++) {
  ...
  //    {
  //        for(size_t j = 0; j <
  //        global_config.controller.controller_output.size(); j++)
  //        {
  //            cout << B[i][j] << " ";
  //        }
  //        cout << endl;
  ...
  matB = mxCreateDoubleMatrix(
      n, global_config.controller.controller_output.size(), mxREAL);
  memcpy((void *)mxGetPr(matB), (void *)B, sizeof(B));
  engPutVariable(ep, "B", matB);

  matC = mxCreateDoubleMatrix(global_config.controller.plant_output.size(), m,
                              mxREAL);
  ...
  stringstream ss;
  ss << "check_stability(A,B,C,D," << T << ","
     << param.get_map()["Kp"].leftBound() << ","
     << param.get_map()["Ki"].leftBound() << ","
     << param.get_map()["Kd"].leftBound() << ");";
