...
bool stability::jury_test(std::vector<double> poly)
{
  ...
  for (size_t j = 0; j < n - 2; j++)
  {
    ...
    {
      new_row.push_back(
        poly.at(m) * poly.at(i + 1) - poly.at(0) * poly.at(m - 1 - i));
    }
    //reverse(new_row.begin(), new_row.end());
    if (abs(new_row.back()) <= abs(new_row.front()))
      return false;
    //reverse(new_row.begin(), new_row.end());
    poly = new_row;
  }
  ...
  box param)
{
  ...
  for (auto it = odes.begin(); it != odes.end(); it++)
  {
    ...
    {
      if (
        (pdrh::node_to_string_infix(it->second) != "0") &&
  ...
  for (auto it = init_map.begin(); it != init_map.end(); it++)
  {
    ...
    else
    {
      ...
      if (
        (pdrh::node_to_string_infix(odes[it->first]) != "0") &&
  ...
  matC = mxCreateDoubleMatrix(
    global_config.controller.plant_output.size(), m, mxREAL);
  memcpy((void *)mxGetPr(matC), (void *)C, sizeof(C));
  engPutVariable(ep, "C", matC);

  matD = mxCreateDoubleMatrix(
    global_config.controller.controller_output.size(),
    global_config.controller.plant_output.size(),
    mxREAL);
