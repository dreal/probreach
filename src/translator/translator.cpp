 ...
 */
void translator::Translator::set_block_param(
  const string subSysHandler,
 ...
 * @param inportCount
 */
void translator::Translator::add_scope_block(
  const string &subSysHandler,
  const string &blkName,
  const unsigned long inportCount)
{
  this->addBlock(
    this->currentSubSystemHandler, "simulink/Sinks/Scope", blkName);
...
void translator::Translator::connect_blocks(
  string subSysHandler,
  translator::block_connection out_block,
...
void translator::Translator::generate_init_var_blocks(const pdrh::mode &m)
{
  for (auto it = m.odes.cbegin(); it != m.odes.cend(); it++)
  {
    ...
    {
      this->addBlock(
        this->currentSubSystemHandler,
        "simulink/Continuous/Integrator",
        it->first);
      this->set_block_param(
        this->currentSubSystemHandler,
        it->first,
        "InitialCondition",
        init_value);
  ...
  block_connection parent_block)
{
  if (expr->operands.size() == 0)
  {
    ...
    else
    {
      ...
      string simulink_name = addBlock(
        this->currentSubSystemHandler,
        "simulink/Commonly Used Blocks/Constant",
        blkName,
        parent_block);
      set_block_param(
        this->currentSubSystemHandler, simulink_name, "Value", expr->value);
    }
  }
  ...
  else
  {
    ...
    if (strcmp(expr->value.c_str(), "+") == 0)
    {
      ...
      translate_ode_expression(
        expr->operands.at(0), block_connection(simulink_name, 1));
      translate_ode_expression(
        expr->operands.at(1), block_connection(simulink_name, 2));
    }
    ...
    else if (strcmp(expr->value.c_str(), "^") == 0)
    {
      ...
      string simulink_name = addBlock(
        ...
        parent_block);
      set_block_param(
        this->currentSubSystemHandler, simulink_name, "Operator", "pow");
    ...
    else if (strcmp(expr->value.c_str(), "log") == 0)
    {
      ...
      string simulink_name = addBlock(
        ...
        parent_block);
      set_block_param(
        this->currentSubSystemHandler, simulink_name, "Operator", "log");
    ...
    else if (strcmp(expr->value.c_str(), "sin") == 0)
    {
      ...
      string simulink_name = addBlock(
        ...
        parent_block);
      set_block_param(
        this->currentSubSystemHandler, simulink_name, "Operator", "sin");
    ...
    else if (strcmp(expr->value.c_str(), "cos") == 0)
    {
      ...
      string simulink_name = addBlock(
        ...
        parent_block);
      set_block_param(
        this->currentSubSystemHandler, simulink_name, "Operator", "cos");
    ...
    else if (strcmp(expr->value.c_str(), "tan") == 0)
    {
      ...
      string simulink_name = addBlock(
        ...
        parent_block);
      set_block_param(
        this->currentSubSystemHandler, simulink_name, "Operator", "tan");
    ...
    else if (strcmp(expr->value.c_str(), "asin") == 0)
    {
      ...
      string simulink_name = addBlock(
        ...
        parent_block);
      set_block_param(
        this->currentSubSystemHandler, simulink_name, "Operator", "asin");
    ...
    else if (strcmp(expr->value.c_str(), "acos") == 0)
    {
      ...
      string simulink_name = addBlock(
        ...
        parent_block);
      set_block_param(
        this->currentSubSystemHandler, simulink_name, "Operator", "acos");
    ...
    else if (strcmp(expr->value.c_str(), "atan") == 0)
    {
      ...
      string simulink_name = addBlock(
        ...
        parent_block);
      set_block_param(
        this->currentSubSystemHandler, simulink_name, "Operator", "atan");
 ...
 */
string
translator::Translator::translate_jump_guard(pdrh::node *guard, int mode_id)
{
...

  {
    ...
    s << translate_jump_guard(
           guard->operands.at(guard->operands.size() - 1), mode_id)
      << ")";
  }
...
void translator::Translator::translate_model()
{
  ...
  this->engine->eval(
    convertUTF8StringToUTF16String("open_system(" + systemHandlerName + ");"));
  ...
  for (const pdrh::mode &m : pdrh::modes)
  {
    ...
    this->add_scope_block(
      this->currentSubSystemHandler, scope_block_name, m.odes.size());
...
string translator::Translator::add_transition(
  const string &parentChartRef,
...
void translator::Translator::add_plant_transitions(const pdrh::mode &mode)
{
  ...
  for (pdrh::mode::jump jump : mode.jumps)
  {
    ...
    if (jump.next_id == mode.id)
    {
      ...
      engine->eval(convertUTF8StringToUTF16String(
        slDummyState + ".Name = '" + slDummyState + "';"));
 ...
 */
string
translator::Translator::controller_jump_guard(pdrh::node *guard, int mode_id)
{
  stringstream s, value;
  if (pdrh::var_map.count(guard->value))
  {
...
{
  engine->eval(
    convertUTF8StringToUTF16String("d = Stateflow.Data(" + parentChart + ");"));
...
void translator::Translator::translate_model_decomposed()
{
  ...
  this->engine->eval(convertUTF8StringToUTF16String("load_system('sflib');"));
  this->engine->eval(convertUTF8StringToUTF16String(
    "add_block('sflib/Chart', ['" + modelName + "' '/" + plant_ref_name +
    "']);"));
  ...
  /**
     ...
     */
  this->engine->eval(convertUTF8StringToUTF16String(
    "add_block('sflib/Chart', ['" + modelName + "' '/" + controller_ref_name +
    "']);"));
  this->engine->eval(convertUTF8StringToUTF16String(
    parentChart +
 ...
 */
string translator::Translator::translate_reset_condition(
  pdrh::mode::jump &jump,
  int source_mode_id)
{
  stringstream reset_condition;
...
void addBlock(
  const unique_ptr<MATLABEngine> &engine,
  string systemHandler,
  string subSysHandler,
  string srcPath,
  string blkName)
{
...
string translator::get_initial_value(string variable_name)
{
  ...
  else
  {
    ...
    CLOG(WARNING, "translator")
      << "Initial condition for " << variable_name
