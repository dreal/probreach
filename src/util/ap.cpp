...
int ap::jumps_per_mode(pdrh::mode *cur_mode, pdrh::mode *prev_mode,
                       vector<box> boxes) {
  ...
  if (prev_mode->id > 0) {
    ...
    left_over = capd::interval(fmod(meal_time_prev_mode.leftBound(),
                                    sample_rate_prev_mode.leftBound()),
                               fmod(meal_time_prev_mode.rightBound(),
                                    sample_rate_prev_mode.rightBound()));
  } else {
  ...
  for (pdrh::state st : pdrh::goal) {
    if (cur_mode->id == st.id) {
      return ceil(((ap::get_meal_time(st.prop, boxes) + left_over) /
                   sample_rate_cur_mode)
                      .rightBound());
    }
  }
  return ceil(
...
box ap::solve_odes(map<string, pdrh::node *> odes, box init,
                   capd::interval time, vector<box> boxes) {
  ...
  var_string.back() = ';';
  fun_string.back() = ';';
  //    cout << "Solving ODEs: " << par_string + var_string + fun_string <<
  //    endl;
...
box ap::solve_odes_nonrig(map<string, pdrh::node *> odes, box init,
                          capd::interval time, vector<box> boxes) {
  // creating capd string here
  // declaring parameters
  string par_string = "par:";
...
// boxes - vector of parameter boxes, init - initial box, time - time horizon
box ap::solve_odes_discrete(map<string, pdrh::node *> odes, box init,
                            capd::interval time, size_t num_points,
                            vector<box> boxes) {
  // creating capd string here
  // declaring parameters
  string par_string = "par:";
...
int ap::simulate_path(vector<pdrh::mode *> path, box init, vector<box> boxes) {
  ...
  if (path.size() == 1) {
    return decision_procedure::check_invariants(
        path.front(), time_to_goal(path.front(), boxes), init, boxes,
        global_config.solver_bin, global_config.solver_opt);
  }
  // reachability depth > 0
  vector<box> sol_box;
  ...
  // CLOG_IF(global_config.verbose, INFO, "algorithm") << "Window size: " <<
  // window_size;
  //    size_t sat_num = 0;
  //    size_t unsat_num = 0;
  //    size_t undet_num = 0;
  ...
  for (size_t j = 0; j < path.size() - 1; j = j + window_size) {
    ...
    for (size_t i = j; (i < j + window_size) && (i < path.size() - 1); i++) {
      ...
      CLOG_IF(global_config.verbose, INFO, "algorithm")
          << "Init box (VERIFIED) in mode " << cur_mode->id << " for depth "
          << i;
      ...
      if (ap::check_invariants(cur_mode, init_box_hull, boxes)) {
        invt_check = decision_procedure::check_invariants(
            cur_mode, time, init_box_hull, boxes, global_config.solver_bin,
      ...
      switch (invt_check) {
        case decision_procedure::SAT:
          CLOG_IF(global_config.verbose, INFO, "algorithm")
              << "Invariant is SAT in mode " << cur_mode->id
              << " @ depth=" << i;
          break;

        case decision_procedure::UNDET:
          CLOG_IF(global_config.verbose, INFO, "algorithm")
              << "Invariant is UNDET in mode " << cur_mode->id
              << " @ depth=" << i;
          return decision_procedure::UNDET;

        case decision_procedure::UNSAT:
          CLOG_IF(global_config.verbose, INFO, "algorithm")
              << "Invariant is UNSAT in mode " << cur_mode->id
              << " @ depth=" << i;
      ...
      for (size_t k = 0; k < init_box.size(); k++) {
        ...
        // sol_box.push_back(solve_odes_nonrig(cur_mode->odes, init_box.at(k),
        // time, boxes));
      }
      CLOG_IF(global_config.verbose, INFO, "algorithm")
      ...
      //            cout << "Solution box hull in mode " << cur_mode->id << " at
      //            depth = " << i << endl; cout <<
      //            box_factory::box_hull(sol_box) << endl;

      //            #pragma omp critical
      //            cout << "Total number of boxes so far: " << sol_box.size()
      //            << endl;
      vector<box> part_sol_box;
      for (box b : sol_box) {
        vector<box> part_boxes = box_factory::bisect(b, vector<string>{"e"});
        part_sol_box.insert(part_sol_box.end(), part_boxes.begin(),
                            part_boxes.end());
      }
      ...
      for (box b : part_sol_box) {
        ...
        //                for (auto it = reset_map.begin(); it !=
        //                reset_map.end(); it++)
        //                {
        //                    if((pdrh::par_map.find(it->first) ==
        //                    pdrh::par_map.end()) &&
        //                       (pdrh::rv_map.find(it->first) ==
        //                       pdrh::rv_map.end()) &&
        //                       (pdrh::dd_map.find(it->first) ==
        //                       pdrh::dd_map.end()))
        //                    {
        //                        vector<box> reset_boxes = boxes;
        //                        reset_boxes.push_back(b);
        //                        init_map.insert(make_pair(it->first,
        //                        pdrh::node_to_interval(it->second,
        //                        reset_boxes)));
        //                    }
        //                }
  ...
  if (ap::check_invariants(path.back(), init_box_hull, boxes)) {
    invt_check = decision_procedure::check_invariants(
        path.back(), time, init_box_hull, boxes, global_config.solver_bin,
        global_config.solver_opt);
  }
  switch (invt_check) {
    case decision_procedure::SAT:
  ...
  for (size_t k = 0; k < init_box.size(); k++) {
    ...
    // sol_box.push_back(solve_odes_nonrig(path.back()->odes, init_box.at(k),
    // time, boxes));
  }
  return decision_procedure::SAT;
}

box ap::compute_objective(vector<pdrh::mode *> path, box init,
                          vector<box> boxes, vector<string> obj_name) {
  // reachability depth == 0
  if (path.size() == 1) {
    box sol = solve_odes(path.front()->odes, init,
                         time_to_goal(path.front(), boxes), boxes);
  ...
  for (size_t j = 0; j < path.size() - 1; j = j + window_size) {
    ...
    for (size_t i = j; (i < j + window_size) && (i < path.size() - 1); i++) {
      ...
      cout << "Solution @ step " << i << " in mode " << cur_mode->id << ": "
           << endl
           << sol << endl;
      //            capd::interval jump_time  =
      //            decision_procedure::get_jump_time(cur_mode, init, boxes);
...
#pragma omp critical
      {
        ...
        //                cout << "--------------------" << endl;
        //                cout << "The domain of system variables: " << domain
        //                << endl; cout << "--------------------" << endl;
        //                if(domain.contains(sol))
        //                {
        //                    cout << "The solution is inside the domain" <<
        //                    endl;
        //                }
        //                else
        //                {
        //                    cout << "The solution is outside the domain" <<
        //                    endl;
        //                }
        //                cout << endl;
      }
...
capd::interval ap::compute_robustness(vector<pdrh::mode *> path, box init,
                                      vector<box> boxes) {
  ...
  // reachability depth == 0
  if (path.size() == 1) {
    box sol = solve_odes_nonrig(path.front()->odes, init,
                                time_to_goal(path.front(), boxes), boxes);

    //        cout << "Solution @ 0: " << sol << endl;
    //        cout << "----------" << endl;
    //
    //        cout << "There are " << path.front()->invts.size() << " invariants
    //        to check:" << endl;
    for (pdrh::node *n : path.front()->invts) {
      //            cout << "Evaluating invariant : " <<
      //            pdrh::node_to_string_infix(n) << endl;
      if (n->value != ">=" || n->operands.size() > 2 ||
  ...
  for (size_t j = 0; j < path.size() - 1; j = j + window_size) {
    ...
    for (size_t i = j; (i < j + window_size) && (i < path.size() - 1); i++) {
      ...
      // solving odes
      // sol = solve_odes_nonrig(cur_mode->odes, init, time, boxes);
      sol = solve_odes_discrete(cur_mode->odes, init, time,
                                global_config.ode_discretisation, boxes);
      // sol = init;
      //            cout << "Solution @ " << i << ": " << sol << endl;
      //            cout << "----------" << endl;
      //
      //            cout << "There are " << cur_mode->invts.size() << "
      //            invariants to check:" << endl;
  ...
  //    cout << "There are " << path.back()->invts.size() << " invariants to
  //    check:" << endl;
  for (pdrh::node *n : path.back()->invts) {
    //        cout << "Evaluating invariant : " << pdrh::node_to_string_infix(n)
    //        << endl;
    if (n->value != ">=" || n->operands.size() > 2 ||
        n->operands.back()->value != "0") {
      ostringstream s;
...
capd::interval ap::compute_max_robustness(vector<vector<pdrh::mode *>> paths,
...
capd::interval ap::compute_min_robustness(vector<vector<pdrh::mode *>> paths,
                                          box init, vector<box> boxes) {
...
int ap::verify(size_t min_depth, size_t max_depth, vector<box> boxes) {
  // list of all evaluated paths, where each path consists is a sequence of
  // pairs (<mode_id>, <init_value>)
  ...
  while (paths.size() > 0) {
    ...
    if (global_time >= pdrh2box::node_to_interval(
    ...
    if (cur_mode->jumps.size() == 0) {
      ...
      int invt_check = decision_procedure::check_invariants(
          cur_mode, time_bound, init, boxes, dreal::solver_bin,
          global_config.solver_opt);
      switch (invt_check) {
        ...
        case decision_procedure::SAT:
          if (global_time + time_bound >=
              ...
              path.size() - 1 >= max_depth) {
            CLOG_IF(global_config.verbose, INFO, "algorithm")
                << "Global time limit has been reached in mode "
                << cur_mode->id;
            return decision_procedure::SAT;
          }
          break;
    ...
    for (pdrh::mode::jump jump : cur_mode->jumps) {
      ...
      if (jump_time.first != capd::interval(-1)) {
        ...
        // cout << "Initial condition: " << init << endl;
        int invt_check = decision_procedure::check_invariants(
            cur_mode, jump_time.first, init, boxes, global_config.solver_bin,
            global_config.solver_opt);
    ...
    // removing the sampling jumps if there are other which are enabled in this
    // mode
    ...
    for (auto it = jumps_times.begin(); it != jumps_times.end(); it++) {
      // box sol = solve_odes_discrete(cur_mode->odes, init, it->second,
      // global_config.ode_discretisation, boxes); box sol =
      // solve_odes_nonrig(cur_mode->odes, init, it->second.leftBound(), boxes);
      ...
      if (capd::intervals::intersection(
              ...
              sol.get_map()[global_config.global_time], intersection)) {
        // considering box hull here then
        sol = ap::apply_reset(
            cur_mode->get_jump(sample_jump.first).reset,
            box_factory::box_hull({sol, sample_jump.second.second}), boxes);
      }
...
box ap::apply_reset(map<string, pdrh::node *> reset_map, box sol,
                    vector<box> boxes) {
  ...
  //    sol_map["e_phi"] += gsl_ran_gaussian_ziggurat(r,
  //    global_config.noise_var); sol_map["e_psi"] +=
  //    gsl_ran_gaussian_ziggurat(r, global_config.noise_var); sol_map["e_the"]
  //    += gsl_ran_gaussian_ziggurat(r, global_config.noise_var); sol_map["e"]
  //    += gsl_ran_gaussian_ziggurat(r, global_config.noise_var);
...
int ap::simulate(size_t min_depth, size_t max_depth, vector<box> boxes) {
  ...
  while (paths.size() > 0) {
    ...
    for (size_t i = 0; i < global_config.ode_discretisation; i++) {
      ...
      if (init.get_map()[global_config.global_time].leftBound() >=
              pdrh2box::node_to_interval(
                  pdrh::var_map[global_config.global_time].second)
                  .rightBound() ||
          path.size() - 1 >= max_depth) {
      ...
      // box sol = solve_odes_nonrig(cur_mode->odes, init, integration_step,
      // boxes); box sol = solve_odes(cur_mode->odes, init, integration_step,
      // boxes);
      ...
      for (pdrh::mode::jump jump : cur_mode->jumps) {
        ...
        if (jumps_times.find(jump.next_id) == jumps_times.end() &&
            !ap::is_sample_jump(jump)) {
          // cout << "Checking jump to mode " << jump.next_id;
          // checking if either of the jumps is enabled
          if (pdrh2box::check_zero_crossing(jump.guard, boxes, init, sol)) {
            // if sample jump is enabled as well then we apply the sampling
            // reset first
            if (sample_jump.first != 0) {
              sol = ap::apply_reset(cur_mode->get_jump(sample_jump.first).reset,
                                    sol, boxes);
            }
    ...
    for (auto it = jumps_times.begin(); it != jumps_times.end(); it++) {
      // cout << "To mode " << it->first << " at time " << it->second.first << "
      // with the initial condition: " << it->second.second << endl;
