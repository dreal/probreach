...
bool box::empty() const { return (get_map().size() == 0); }
...
box::box(string line) {
  ...
  while (line.length() > 0) {
    ...
    b_map.insert(make_pair(
        var,
        capd::interval(interval_string.substr(1, pos3 - 1),
...
box box::get_stddev() {
  ...
  for (auto it = edges.cbegin(); it != edges.cend(); it++) {
    sigma_map.insert(make_pair(
        it->first, capd::intervals::width(capd::interval(
                       it->second.leftBound(), it->second.mid().leftBound()))));
  }
...
box box::fmod(int mod) {
  ...
  for (auto it = this->edges.begin(); it != this->edges.end(); it++) {
    res.insert(
        make_pair(it->first, this->edges[it->first].leftBound() -
                                 (long)this->edges[it->first].leftBound()));
  }
  return box(res);
}

void box::erase(string var) { this->edges.erase(var); }