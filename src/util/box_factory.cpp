...
vector<box> box_factory::bisect(box b, map<std::string, capd::interval> e) {
  ...
  for (auto it = m.cbegin(); it != m.cend(); it++) {
    if (capd::intervals::width(it->second) > e[it->first].leftBound()) {
      ...
      tmp_v.push_back(capd::interval((it->second).leftBound(),
                                     (it->second).mid().rightBound()));
      tmp_v.push_back(capd::interval((it->second).mid().leftBound(),
...
vector<box> box_factory::bisect(box b, map<string, pdrh::node*> e) {
  ...
  for (auto it = m.cbegin(); it != m.cend(); it++) {
    ...
    if (capd::intervals::width(it->second) >
...
box box_factory::merge(box lhs, box rhs) {
  ...
  if (m[neq_dim].rightBound() == rhs.get_map()[neq_dim].leftBound()) {
    m[neq_dim] = capd::interval(m[neq_dim].leftBound(),
                                rhs.get_map()[neq_dim].rightBound());
    return box(m);
  } else {
    if (m[neq_dim].leftBound() == rhs.get_map()[neq_dim].rightBound()) {
      m[neq_dim] = capd::interval(rhs.get_map()[neq_dim].leftBound(),
                                  m[neq_dim].rightBound());
      return box(m);
    } else {
      return box();
    }
...
box_factory::get_intersection_conflicts(map<box, capd::interval> original,
