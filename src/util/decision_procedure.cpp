 ...
 * @return satisfiability of reachability for the given set of paths and
 * parameter boxes.
 */
int decision_procedure::evaluate(vector<vector<pdrh::mode *>> paths,
                                 vector<box> boxes, string solver_bin,
                                 string solver_opt) {
  int undet_counter = 0;
 ...
 * @param solver_bin - full path to solver binary.
 * @param solver_opt - string of solver options.
 * @return satisfiability of reachability for the given path and parameter
 * boxes.
 */
int decision_procedure::evaluate(vector<pdrh::mode *> path, vector<box> boxes,
                                 string solver_bin, string solver_opt) {
 ...
 * @param solver_bin - full path to solver binary.
 * @param solver_opt - string of solver options.
 * @return delta-satisfiability of reachability for the given path and parameter
 * boxes.
 */
int decision_procedure::evaluate_delta_sat(vector<pdrh::mode *> path,
                                           vector<box> boxes, string solver_bin,
                                           string solver_opt) {
  ...
  //    smt_file << pdrh2box::reach_to_smt2(pdrh::init.front(),
  //    pdrh::goal.front(), path, boxes);
  smt_file << smt2_generator::reach_to_smt2(path, boxes);
  smt_file.close();
  //    cout << pdrh2box::reach_to_smt2(pdrh::init.front(), pdrh::goal.front(),
  //    path, boxes) << endl; exit(EXIT_SUCCESS);
  ...
  if (first_res == -1) {
    ...
    // //else
    // {
    //     cerr << "Problem occurred while removing one of auxiliary files
    //     (UNSAT)"; return decision_procedure::ERROR;
    // }
  } else {
    // if((std::remove(smt_filename.c_str()) == 0) &&
    //   (std::remove(std::string(smt_filename + ".output").c_str()) == 0))
    {
      ...
      //            pdrh::modes.front().jumps.front().reset; map<string,
      //            capd::interval> init_map; for(auto it = reset_map.begin();
      //            it != reset_map.end(); it++)
      //            {
      //                init_map.insert(make_pair(it->first,
      //                pdrh::node_to_interval(it->second, b)));
      //            }
      //            box init_box(init_map);
      //            cout << "New init box: " << init_box << endl;
      //            pdrh::state new_init_state;
      //            new_init_state.id =
      //            pdrh::modes.front().jumps.front().next_id;
      //            new_init_state.prop = pdrh::box_to_node(init_box);
      //            cout << "New init state: " << new_init_state.id << ": " <<
      //            pdrh::node_to_string_prefix(new_init_state.prop) << endl;
      // LOG(DEBUG) << "Removed auxiliary files";
    ...
    // {
    //     cerr << "Problem occurred while removing one of auxiliary files
    //     (DELTA-SAT)"; return decision_procedure::ERROR;
    // }
  }
}
 ...
 * @param solver_bin - full path to solver binary.
 * @param solver_opt - string of solver options.
 * @return delta-satisfiability of non-reachability for the given path and
 * parameter boxes.
 */
int decision_procedure::evaluate_complement(vector<pdrh::mode *> path,
                                            vector<box> boxes,
                                            string solver_bin,
                                            string solver_opt) {
  int thread_num = 0;
...
/**
 * Checks mode invariants given an initial state, set of parameter boxes and a
 * time bound.
 *
 * @param m - mode.
 ...
 */
int decision_procedure::check_invariants(pdrh::mode *m, capd::interval time,
...
/**
 * Computes the time of the given jump from the current mode for a given initial
 * value and a set of parameter boxes.
 *
 * @param m - mode.
...
pair<capd::interval, box> decision_procedure::get_jump_time(
    pdrh::mode *m, pdrh::mode::jump jump, box init, vector<box> boxes) {
  // default value for the thread number
  int thread_num = 0;
#ifdef _OPENMP
  thread_num = omp_get_thread_num();
  ...
  res = dreal::execute(global_config.solver_bin, smt_filename,
