...
int formal::evaluate_ha(int min_depth, int max_depth) {
  ...
  return decision_procedure::evaluate(paths, {}, global_config.solver_bin,
...
capd::interval formal::evaluate_pha(int min_depth, int max_depth) {
  ...
  CLOG_IF(global_config.verbose, INFO, "algorithm")
      << "Domain of continuous random parameters: " << rv_domain;
  ...
  for (box dd : dd_partition) {
    if (pdrh::rv_map.size() > 0)
      probability = capd::interval(
          0, 2 - measure::p_measure(rv_domain, global_config.precision_prob)
                     .leftBound());
    ...
    while (capd::intervals::width(probability) > global_config.precision_prob) {
      ...
      if (global_config.sort_rv_flag) {
        CLOG_IF(global_config.verbose, INFO, "algorithm")
            << "Sorting the partition of domain of continuous random "
               "parameters";
        sort(rv_partition.begin(), rv_partition.end(),
...
#pragma omp critical
          {
            ...
            switch (res) {
              case decision_procedure::SAT:
                ...
                // if(capd::intervals::width(probability) <=
...
#pragma omp critical
        {
          ...
          if (!sat_flag) {
            ...
            if (undet_counter > 0) {
              ...
              rv_stack.insert(rv_stack.end(), rv_bisect.begin(),
                              rv_bisect.end());
            }
...
std::map<box, capd::interval> formal::evaluate_npha(int min_depth,
                                                    int max_depth) {
  CLOG_IF(global_config.verbose, INFO, "algorithm")
  ...
  if (global_config.partition_nondet) {
    ...
    nd_partition.clear();
    // nd_partition = box_factory::partition(nd_domain,
    // global_config.precision_nondet);
    nd_partition =
  ...
  // performing extra partition if the probability partition map is defined
    ...
    // cout << "rv partition size before extra partition: " <<
    // rv_partition.size() << endl;
    ...
    for (box b : rv_partition) {
      ...
      tmp_vector.insert(tmp_vector.cend(), extra_partition.cbegin(),
                        extra_partition.cend());
    }
    rv_partition = tmp_vector;
    // cout << "rv partition size after extra partition:" << rv_partition.size()
    // << endl;
  }
  // sorting boxes by probability value
  if (global_config.sort_rv_flag) {
  ...
  capd::interval total_probability = capd::interval(
      0, 2 - measure::p_measure(rv_domain, global_config.precision_prob)
                 .leftBound());
...
#pragma omp critical
        ...
        for (int i = 0; i < rv_partition.size(); i++) {
          box rv = rv_partition.at(i);
          // calculating probability measure of the box rv; initally p_box =
          // [1.0, 1.0]
...
#pragma omp critical
          ...
          switch (decision_procedure::evaluate(paths, vector<box>{nd, dd, rv},
...
#pragma omp critical
            {
              if (p_box.leftBound() > 0) {
                p_map[nd] =
                    capd::interval(p_map[nd].leftBound() + p_box.leftBound(),
                                   p_map[nd].rightBound());
              }
              CLOG_IF(global_config.verbose, INFO, "algorithm") << "SAT";
              CLOG_IF(global_config.verbose, INFO, "algorithm")
...
#pragma omp critical
            {
              if (p_box.leftBound() > 0) {
                p_map[nd] =
                    capd::interval(p_map[nd].leftBound(),
...
#pragma omp critical
            {
              ...
              auto it =
                  find(total_partition.begin(), total_partition.end(), rv);
              if (it != total_partition.end()) {
                total_partition.erase(it);
                total_partition.insert(total_partition.end(), rv_bisect.begin(),
                                       rv_bisect.end());
              }
            } break;
          }
...
#pragma omp critical
        {
          ...
          if (global_config.sort_rv_flag) {
            ...
            sort(rv_stack.begin(), rv_stack.end(),
                 measure::compare_boxes_by_p_measure);
          }
          // updating partition map only in case if probability value does not
          // satisfy the probability precision
          CLOG_IF(global_config.verbose, INFO, "algorithm")
          ...
          //}
          //                cout << "Probability map after " << nd << " was
          //                processed" << endl; for(auto it2 = p_map.begin();
          //                it2 != p_map.end(); it2++)
          //                {
          //                    std::cout << it2->first << " | " << it2->second
          //                    << std::endl;
          //                }
          //                cout << "Resulting map after " << nd << " was
          //                processed" << endl; for(auto it2 = res_map.begin();
          //                it2 != res_map.end(); it2++)
          //                {
          //                    std::cout << it2->first << " | " << it2->second
          //                    << std::endl;
      ...
      //            cout << "----------------------------------" << endl;
      //            cout << "Probability map after all boxes are processed" <<
      //            endl; for(auto it2 = p_map.begin(); it2 != p_map.end();
      //            it2++)
      //            {
      //                std::cout << it2->first << " | " << it2->second <<
      //                std::endl;
      //            }
      //            cout << "Resulting map after all boxes are processed" <<
      //            endl; for(auto it2 = res_map.begin(); it2 != res_map.end();
      //            it2++)
      //            {
      //                std::cout << it2->first << " | " << it2->second <<
      //                std::endl;
    ...
    //        }
    //        cout << "Resulting map before it's been multiplied by dd measure"
    //        << endl; for(auto it2 = res_map.begin(); it2 != res_map.end();
    //        it2++)
    //        {
    ...
    //        }
    //        exit(EXIT_FAILURE);
    //        cout << "Updated probability map after the end of the loop" <<
    //        endl; for(auto it2 = p_map.begin(); it2 != p_map.end(); it2++)
    //        {
    //            std::cout << it2->first << " | " << it2->second << std::endl;
    //        }
  }
