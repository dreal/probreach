...
capd::interval algorithm::evaluate_pha_chernoff(int min_depth, int max_depth,
                                                double acc, double conf,
  ...
  //    long int sample_size = algorithm::get_cernoff_bound(acc,
  //    std::sqrt(conf));
  long int sat = 0;
  long int unsat = 0;
...
#pragma omp parallel for schedule(dynamic)
  for (long int ctr = 0; ctr < sample_size; ctr++) {
    ...
    //                    std::vector<std::vector<pdrh::mode *>> paths_j =
    //                    pdrh::get_paths(pdrh::get_mode(i.id),
    //                                                                                     pdrh::get_mode(g.id), j);
    //                    paths.insert(paths.cend(), paths_j.cbegin(),
    //                    paths_j.cend());
    //                }
    //            }
    //        }
...
capd::interval algorithm::evaluate_pha_bayesian(int min_depth, int max_depth,
                                                double acc, double conf,
                                                vector<box> nondet_boxes) {
  const gsl_rng_type *T;
  gsl_rng *r;
...
#pragma omp critical
    ...
    switch (global_config.decision_method) {
      ...
      case 2:
        ...
        //                cout << "Post prob " << post_prob << " required conf "
        //                << conf << endl; cout << "Accuracy " << acc << endl;
        //                cout << "SAT " << sat << endl;
...
#pragma omp critical
    {
      ...
      if (post_mean_sat >= acc) {
        post_prob = gsl_cdf_beta_P(post_mean_unsat + acc,
                                   sample_size - unsat + alpha, unsat + beta) -
                    gsl_cdf_beta_P(post_mean_sat - acc, sat + alpha,
                                   sample_size - sat + beta);
      } else {
        post_prob = gsl_cdf_beta_P(post_mean_unsat + acc,
                                   sample_size - unsat + alpha, unsat + beta) -
      ...
      CLOG_IF(global_config.verbose, INFO, "algorithm")
          << "CI: "
          << capd::interval(max(post_mean_sat - acc, 0.0),
  ...
  return capd::interval(max(post_mean_sat - acc, 0.0),
                        min(post_mean_unsat + acc, 1.0));
}

// pair<box, capd::interval> algorithm::evaluate_npha_sobol(int min_depth, int
// max_depth, int size) {
...
//        CLOG_IF(global_config.verbose_result, INFO, "algorithm") << "Explored
//        space: " << domain << " | "
//                                                                 <<
...
//            CLOG_IF(global_config.verbose_result, INFO, "algorithm") <<
//            "Quasi-random sample: " << b; capd::interval probability; if
//            (global_config.bayesian_flag) {
//                probability = evaluate_pha_bayesian(min_depth, max_depth,
//                global_config.bayesian_acc,
//                                                    global_config.bayesian_conf,
//                                                    vector<box>{b});
//            } else if (global_config.chernoff_flag) {
//                probability = evaluate_pha_chernoff(min_depth, max_depth,
...
//            measure::compare_pairs::descending);
//        } else {
//            sort(samples.begin(), samples.end(),
//            measure::compare_pairs::ascending);
//        }
//        vector<pair<box, capd::interval>> elite;
//        copy_n(samples.begin(), ceil(samples.size() *
//        global_config.elite_ratio), back_inserter(elite)); vector<box>
//        elite_boxes; for (pair<box, capd::interval> p : elite) {
...
pair<box, capd::interval> algorithm::evaluate_npha_cross_entropy_normal(
    size_t min_depth, size_t max_depth, size_t size, size_t iter_num,
    double acc, double conf) {
  return evaluate_npha_cross_entropy_normal(min_depth, max_depth, size,
                                            iter_num, acc, conf, NULL);
}
...
pair<box, capd::interval> algorithm::evaluate_npha_cross_entropy_normal(
  ...
  for (int j = 0; j < iter_num; j++) {
    ...
    CLOG_IF(global_config.verbose_result, INFO, "algorithm")
        << "Variance: " << var;
    // correct the sample size only if the probability of sampling outside the
    // domain is still greater than 0.99999
    ...
    for (int i = 0; i < new_size; i++) {
      ...
      if (domain.contains(b)) {
        ...
        if (resume) {
          ...
          //                    CLOG_IF(global_config.verbose_result, INFO,
          //                    "algorithm") << "Stability check is switched
          //                    off"; if (global_config.bayesian_flag) {
          //                        probability =
          //                        evaluate_pha_bayesian(min_depth, max_depth,
          //                        acc, conf, vector<box>{b});
          //                    } else if (global_config.chernoff_flag) {
          //                        probability =
          //                        evaluate_pha_chernoff(min_depth, max_depth,
          //                        acc, conf, vector<box>{b});
          //                    } else {
          //                        CLOG(ERROR, "algorithm") << "Unknown
          //                        setting"; exit(EXIT_FAILURE);
          //                    }
          // bayesian estimations algorithm by default
          probability = evaluate_pha_bayesian(min_depth, max_depth, acc, conf,
                                              vector<box>{b});
          // fixing probability value
          if (probability.leftBound() < 0) {
            probability.setLeftBound(0);
    ...
    vector<pair<box, capd::interval>> elite;
    copy_n(samples.begin(), ceil(samples.size() * global_config.elite_ratio),
           back_inserter(elite));
    // getting elite boxes
    ...
    if (global_config.min_prob) {
      if (samples.front().second.mid().leftBound() <
          res.second.mid().leftBound()) {
        res = samples.front();
      }
    } else {
      if (samples.front().second.mid().leftBound() >
