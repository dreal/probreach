...
std::pair<capd::interval, std::vector<capd::interval>> measure::integral(
    std::string var, std::string fun, capd::interval it, double e) {
    ...
    if (pdrh::rv_map.find(it->first) != pdrh::rv_map.cend()) {
      res *=
          measure::integral(
              ...
              it->second, measure::precision(e, edges.size()))
              .first;
      // res *= measure::integral(it->first, measure::rv_map[it->first],
      // it->second, power(e, 1/edges.size())).first;
...
capd::interval measure::get_sample_prob(box domain, box mean, box sigma) {
  ...
  for (auto it = edges.begin(); it != edges.end(); it++) {
    ...
    if (pdrh::par_map[it->first].first->value !=
      ...
      pair<capd::interval, vector<capd::interval>> itg = measure::integral(
          it->first,
          measure::distribution::gaussian(it->first, mean.get_map()[it->first],
                                          sigma.get_map()[it->first]),
          it->second, prec);
      res *= itg.first;
    }
  }
...
capd::interval measure::bounds::gaussian(double mu, double sigma, double e) {
  ...
  std::pair<capd::interval, std::vector<capd::interval>> itg =
      measure::integral("x", measure::distribution::gaussian("x", mu, sigma), i,
  ...
  while (1 - itg.first.leftBound() > global_config.integral_inf_coeff * e) {
    ...
    itg =
        measure::integral("x", measure::distribution::gaussian("x", mu, sigma),
                          i, global_config.integral_inf_coeff * e);
  }
  return i;
}
...
capd::interval measure::bounds::exp(double lambda, double e) {
  ...
  std::pair<capd::interval, std::vector<capd::interval>> itg =
      measure::integral("x", measure::distribution::exp("x", lambda), i,
                        (1 - global_config.integral_inf_coeff) * e);
  while (1 - itg.first.leftBound() > global_config.integral_inf_coeff * e) {
    i = capd::interval(0, i.rightBound() + (1 / lambda));
    itg = measure::integral("x", measure::distribution::exp("x", lambda), i,
                            global_config.integral_inf_coeff * e);
  }
  return i;
}

std::pair<capd::interval, std::vector<capd::interval>> measure::bounds::pdf(
    std::string var, std::string pdf, capd::interval domain, double start,
    double e) {
  ...
  while (true) {
    ...
    res = capd::interval(res.leftBound() - global_config.integral_pdf_step,
    ...
    std::pair<capd::interval, std::vector<capd::interval>> itg =
        measure::integral(var, pdf, res, e * global_config.integral_inf_coeff);
    // std::cout << res << " : " << itg.first << " | " <<
    // capd::intervals::width(itg.first) <<  std::endl;
...
std::string measure::distribution::gaussian(std::string var, capd::interval mu,
                                            capd::interval sigma) {
  std::stringstream s;
...
std::vector<box> measure::get_rv_partition() {
  ...
  for (auto it = pdrh::rv_map.cbegin(); it != pdrh::rv_map.cend(); it++) {
    ...
    capd::interval init_domain(-numeric_limits<double>::infinity(),
    ...
    std::pair<capd::interval, std::vector<capd::interval>> bound =
        measure::bounds::pdf(
            it->first, pdrh::node_to_string_infix(get<0>(it->second)),
            init_domain,
            pdrh2box::node_to_interval(get<3>(it->second)).mid().leftBound(),
            measure::precision(global_config.precision_prob,
                               pdrh::rv_map.size()));
    // cout << "Variable = " << it->first << " PDF = " <<
    // pdrh::node_to_string_infix(get<0>(it->second)) << endl;

    // cout << "After bound" << endl;
    // updating rv bounds
    pdrh::rv_map[it->first] =
        make_tuple(std::get<0>(it->second), new node(bound.first.leftBound()),
                   new node(bound.first.rightBound()), get<3>(it->second));
    // updating var bounds
    pdrh::var_map[it->first] = make_pair(new node(bound.first.leftBound()),
...
box measure::bounds::get_rv_domain() {
  ...
  // }
  // cout << scientific << "Probability sum = " << prob_sum << "\t\t|\t\t" <<
  // width(prob_sum) << endl; exit(EXIT_FAILURE);
...
bool measure::compare_pairs::ascending(const pair<box, capd::interval> &lhs,
...
// comparing the medians of the intervals
bool measure::compare_pairs::descending(const pair<box, capd::interval> &lhs,
                                        const pair<box, capd::interval> &rhs) {
  if (!box_factory::get_keys_diff(lhs.first, rhs.first).empty() ||
      !box_factory::get_keys_diff(rhs.first, lhs.first).empty()) {
    stringstream s;
