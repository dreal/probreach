...
void pdrh::push_var(string var, pdrh::node* left, pdrh::node* right) {
  ...
  //    capd::interval domain(-numeric_limits<double>::infinity(),
  //    numeric_limits<double>::infinity()); if(strcmp(left->value.c_str(),
  //    "-infty") != 0)
  //    {
...
void pdrh::push_time_bounds(pdrh::node* left, pdrh::node* right) {
  // COMMENTED OUT UNTIL ALL DEPENDENCIES HAS BEEN RESOLVED
  //    capd::interval domain(pdrh::node_to_interval(left).leftBound(),
  //    pdrh::node_to_interval(right).rightBound());
  //    if(capd::intervals::width(domain) < 0)
  //    {
  //        stringstream s;
...
void pdrh::push_mode(pdrh::mode m) {
  ...
  for (string var : extra_vars) {
    ...
    if (pdrh::var_map[var].first != pdrh::var_map[var].second) {
      ...
      //                       node_to_interval(pdrh::var_map[var].second).rightBound()))
      //                       > 0)
      {
...
void pdrh::push_ode(pdrh::mode& m, string var, pdrh::node* ode) {
  if (pdrh::var_map.find(var) != pdrh::var_map.cend()) {
    if (m.flow_map.find(var) == m.flow_map.cend()) {
      ...
      // removing a variable from the parameter list if there is an ode defined
      // for it
      if (pdrh::par_map.find(var) != pdrh::par_map.cend()) {
...
// adding a reset
void pdrh::push_reset(pdrh::mode& m, pdrh::mode::jump& j, string var,
                      pdrh::node* expr) {
...
// adding a jump
void pdrh::push_jump(pdrh::mode& m, mode::jump j) { m.jumps.push_back(j); }

// adding init
void pdrh::push_init(vector<pdrh::state> s) { pdrh::init = s; }

// adding goal
void pdrh::push_goal(vector<pdrh::state> s) { pdrh::goal = s; }

void pdrh::push_path(vector<mode*> path) { pdrh::paths.push_back(path); }
...
void pdrh::push_rv(string var, pdrh::node* pdf, pdrh::node* left,
                   pdrh::node* right, pdrh::node* start) {
...
vector<pdrh::mode*> pdrh::get_shortest_path(pdrh::mode* begin,
                                            pdrh::mode* end) {
  ...
  if (begin == end) {
    ...
    while (!paths.empty()) {
      ...
      if (!successors.empty()) {
        ...
        if (std::find(successors.begin(), successors.end(), end) !=
            successors.cend()) {
          path.push_back(end);
          paths.clear();
          return path;
...
vector<vector<pdrh::mode*>> pdrh::get_paths(pdrh::mode* begin, pdrh::mode* end,
                                            int path_length) {
  // initializing the set of paths
  vector<std::vector<pdrh::mode*>> paths;
  vector<pdrh::mode*> path;
  ...
  while (!stack.empty()) {
    ...
    // proceeding only if the length of the current path is ascending then the
    // required length
...
// getting all paths of length path_length for all combinations of init and goal
// modes
...
string pdrh::model_to_string() {
  ...
  //    if(!pdrh::is_node_empty(pdrh::time.first) &&
  //    !pdrh::is_node_empty(pdrh::time.second))
  //    {
  //        out << "TIME DOMAIN:" << endl;
  //        out << "|   [" << pdrh::node_to_string_prefix(pdrh::time.first) <<
  //        ", " << pdrh::node_to_string_prefix(pdrh::time.second) << "]" <<
  //        endl;
  //    }
  out << "MODES:" << endl;
  for (pdrh::mode m : pdrh::modes) {
    ...
    for (pdrh::mode::jump j : m.jumps) {
      ...
      for (auto it = j.reset_nondet.cbegin(); it != j.reset_nondet.cend();
           it++) {
        out << "|   |   |   " << it->first << " := ["
            << pdrh::node_to_string_prefix(it->second.first) << ", "
            << pdrh::node_to_string_prefix(it->second.second) << "]" << endl;
...
/**
 * Translates an initial state specified as (and (x1 = v1) ... (xn = vn)) into
 * map<string, double>
 *
 * @param init
 * @return
...
void pdrh::push_psy_goal(int mode_id, box b)
{
    ...
    pdrh::var_map.insert(make_pair("tau",
make_pair(pdrh::push_operation_node("0"), m["tau"])));
...
void pdrh::push_psy_c_goal(int mode_id, box b)
{
    ...
    std::stringstream s;
    s << m["tau"].leftBound();
    pdrh::node *left_time_bound = pdrh::push_operation_node("=",
std::vector<pdrh::node*>{ pdrh::push_terminal_node("tau"),
pdrh::push_terminal_node(s.str())}); s.str("");
    //s << m["tau"].rightBound();
    //pdrh::node *right_time_bound = pdrh::push_operation_node("<=",
std::vector<pdrh::node*>{ pdrh::push_terminal_node("tau"),
pdrh::push_terminal_node(s.str())});
    //st.prop = pdrh::push_operation_node("and",
std::vector<pdrh::node*>{left_time_bound, right_time_bound, or_node}); st.prop =
pdrh::push_operation_node("and", std::vector<pdrh::node*>{left_time_bound,
or_node}); pdrh::goal = std::vector<pdrh::state>{ st };
    // updating time bounds
...
// mode, step, box
/*
vector<tuple<int, box>> pdrh::series_to_boxes(map<string,
vector<capd::interval>> time_series)
{
...
(strcmp(it->first.c_str(), "Step") != 0))
        ...
        res.push_back(std::make_tuple((int)
time_series["Mode"].at(i).leftBound(), box(m)));
    }
    return res;
}
*/
...
/*
vector<pdrh::state> pdrh::series_to_goals(map<string, vector<pair<pdrh::node*,
pdrh::node*>>> time_series)
{
    ...
    for(int i = 0; i < time_series.cbegin()->second.size(); i++)
    {
        ...
        for(auto it = time_series.cbegin(); it != time_series.cend(); it++)
        {
            if(strcmp(it->first.c_str(), "Step") != 0)
            {
                ...
                {
                    istringstream
is(pdrh::node_to_string_prefix(it->second.at(i).first)); is >> goal.id;
                }
                else if(strcmp(it->first.c_str(), "Time") == 0)
                {
                    //pdrh::node* left_node = pdrh::push_operation_node(">=",
{push_terminal_node(global_config.time_var_name), it->second.at(i).first});
                    //pdrh::node* right_node = pdrh::push_operation_node("<=",
{push_terminal_node(global_config.time_var_name), it->second.at(i).second});
                    //operands.push_back(pdrh::push_operation_node("and",
{left_node, right_node})); operands.push_back(pdrh::push_operation_node("=",
{push_terminal_node(global_config.time_var_name), it->second.at(i).first}));
                }
                else
                {
                    ...
                    {
                        pdrh::node* left_node = pdrh::push_operation_node(">=",
{push_terminal_node(it->first), it->second.at(i).first}); pdrh::node* right_node
= pdrh::push_operation_node("<=", {push_terminal_node(it->first),
it->second.at(i).second}); operands.push_back(pdrh::push_operation_node("and",
...
vector<pdrh::mode*> pdrh::get_psy_path(
  ...
  for (int i = 1; i < time_series.cbegin()->second.size(); i++) {
    if (pdrh::node_to_string_prefix(time_series["Mode"].at(i).first) !=
...
/*
std::vector<pdrh::mode*> pdrh::get_psy_path(std::map<std::string,
std::vector<capd::interval>> time_series)
{
    std::vector<pdrh::mode*> path;
    path.push_back(pdrh::get_mode(pdrh::init.front().id));
    for(int i = 1; i < time_series.cbegin()->second.size(); i++)
    {
        if(time_series["Mode"].at(i).leftBound() != time_series["Mode"].at(i -
1).leftBound())
        {
            path.push_back(pdrh::get_mode((int)
time_series["Mode"].at(i).leftBound()));
        }
    }
    return path;
}
...
void pdrh::distribution::push_uniform(string var, pdrh::node* a,
                                      pdrh::node* b) {
  pdrh::distribution::uniform.insert(make_pair(var, make_pair(a, b)));
}

void pdrh::distribution::push_normal(string var, pdrh::node* mu,
                                     pdrh::node* sigma) {
...
pdrh::node* pdrh::distribution::normal_to_node(string var, node* mu,
                                               node* sigma) {
...
void pdrh::set_model_type() {
  if (pdrh::rv_map.empty() && pdrh::dd_map.empty() && pdrh::par_map.empty() &&
      pdrh::syn_map.empty()) {
    pdrh::model_type = pdrh::type::HA;
    // pdrh::model_type = pdrh::type::NPHA;
  } else if (pdrh::rv_map.empty() && pdrh::dd_map.empty() &&
             pdrh::par_map.empty()) {
 ...
 * @param os
 */
void pdrh::output_traj(std::vector<std::map<std::string, double>> traj,
                       std::ostream& os) {
  os << "[" << endl;
