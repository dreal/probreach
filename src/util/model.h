...
struct mode {
  ...
  struct jump {
    ...
    std::map<std::string, std::tuple<std::string, pdrh::node*, pdrh::node*,
                                     pdrh::node*, pdrh::node*>>
        reset_rv;
    ...
    inline jump(const int next_id, node* guard,
                std::map<std::string, node*> reset)
...
// first argument is the address of the mode, second argument is the variable
// name, third argument is the ode
void push_ode(mode&, std::string, node*);
void push_jump(mode&, mode::jump);
// first argument is the address of the jump, second argument is the variable
// name, third argument is the reset expression
...
//    std::vector<mode*> get_psy_path(std::map<std::string,
//    std::vector<capd::interval>>);
std::vector<mode*> get_psy_path(
    std::map<std::string, std::vector<std::pair<pdrh::node*, pdrh::node*>>>);

//    vector<tuple<int, box>> series_to_boxes(std::map<std::string,
//    std::vector<capd::interval>>);
// vector<state> series_to_goals(map<string, vector<pair<pdrh::node*,
// pdrh::node*>>>);
