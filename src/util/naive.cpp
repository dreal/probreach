...
/**
 * Solve an IVP using the first term of Taylor series. This method performs a
...
std::map<std::string, double> naive::solve_ivp(
    std::map<std::string, pdrh::node*> odes, std::map<std::string, double> init,
...
/**
 * Solve an IVP using the first term of Taylor series and record the entire
 * trajectory up to the specified time bound.
 *
 * @param odes - ODE system.
 * @param init - initial value vector.
 * @param param - vector of parameter values.
...
std::vector<std::map<std::string, double>> naive::trajectory(
  ...
  //    for(auto it = init.begin(); it != init.end(); it++) init_map[it->first]
  //    = node_to_double(it->second);
  // if step is greater than time, then step becomes time
  if (dt > time) dt = time;
  // integration time
  double t = 0;
 ...
 * @return
 */
void naive::simulate(std::vector<pdrh::mode> modes,
                     std::vector<pdrh::state> init,
                     std::vector<pdrh::state> goal, bool verify,
                     size_t min_depth, size_t max_depth, size_t max_paths,
                     size_t num_points, std::ostream& os) {
  //    simulate(modes, init, goal, verify, min_depth, max_depth, max_paths,
  //    node_to_double(cur_mode.time.second) / num_points, os);
}
...
void naive::simulate(std::vector<pdrh::mode> modes,
                     ...
                     std::vector<pdrh::state> goal, bool verify,
                     size_t min_depth, size_t max_depth, size_t max_paths,
                     double ode_step, std::ostream& os) {
  // the main path queue
  ...
  while (paths.size() > 0) {
    ...
    vector<map<string, double>> traj =
        trajectory(cur_mode.odes, init_map,
                   node_to_double(cur_mode.time.second), ode_step);
    ...
    for (size_t i = 0; i < traj.size(); i++) {
      ...
      //            }
      // the verify flag is up, hence we need to check the invariants and the
      // goals
      if (verify) {
        ...
        // checking goal conditions for the paths which length is within the
        // required bounds
      ...
      for (mode::jump j : cur_mode.jumps) {
        ...
        if (node_to_boolean(j.guard, sol)) {
          ...
          // adding the computed trajectory to the end of the current path
          path.insert(path.end(), traj.begin(), traj.begin() + i + 1);
          // reassigning the value for all the variables without any specific
          // resets
          init_map = sol;
          // applying the specified resets
          for (auto it = init_map.begin(); it != init_map.end(); it++)
            // the variable is not reset; its current value is carried to the
            // next mode
          ...
          // pushing the path into the set of all paths if their total number is
          // below the defined limit and the length of the newly obtained path
          // is smaller than the provided maximum depth
          if (paths.size() <= max_paths && init_map[".step"] <= max_depth)
            paths.push_back(new_path);
          // we assume that as soon as jump takes place we stop considering this
          // trajectory despite the fact that the corresponding jump can be
          // satisfied multiple times
        }
      }
      cout << "Number of paths: " << paths.size() << endl;
    }
    // printing out a message saying that no jumps can be made from the current
    // mode
    if (!jump_enabled) {
  ...
  if (verify) {
    // if we made it here in "verification" mode, then neither of the goal
