...
/**
 * Getting a string representation of the node in prefix notation with the fixed
 * index.
 *
 * @param n - pointer to the root of the expression tree.
...
string pdrh::node_fix_index(pdrh::node* n, int step, string index) {
  ...
  if (n->operands.size() > 0) {
    ...
    // only performing a soft check here whether the value is digit or an
    // identifier
    if (isdigit(n->value.front()) || n->value == "true" ||
        n->value == "false") {
      s << " " << n->value;
    } else {
...
/**
 * Computes the value of the node provided as the first arguments at the point
...
double pdrh::node_to_double(pdrh::node* n, std::map<std::string, double> vals) {
  // terminal node
  if (n->operands.size() == 0) {
    // returning a value only if the variable from the node appears in the
    // vector of values
  ...
  // operation node
    ...
    if (n->value == "+") {
      ...
      return std::pow(node_to_double(n->operands.front(), vals),
                      node_to_double(n->operands.back(), vals));
...
/**
 * Returns true if zero-crossing happens between the left-handside and the
 * right-handside points.
 *
 *
 ...
 */
bool pdrh::node_zero_crossing(pdrh::node* expr,
  ...
  if (expr->value == ">=" || expr->value == ">" || expr->value == "=" ||
    ...
    //        cout << "Left:" << endl;
    //        for(auto it = left.begin(); it != left.end(); it++) cout <<
    //        it->first << ": " << it->second << endl; cout << "Right:" << endl;
    //        for(auto it = right.begin(); it != right.end(); it++) cout <<
    //        it->first << ": " << it->second << endl; cout <<
    //        (node_to_double(expr->operands.front(), left) -
    //        node_to_double(expr->operands.back(), left)) << " | " <<
    //                    (node_to_double(expr->operands.front(), right) -
    //                    node_to_double(expr->operands.back(), right)) << endl;
    //        cout << "==========" << endl;
...
/**
 * Returns the first node matching the pattern (root->value == values[i]) and
 * (expr).
 *
 ...
 */
void pdrh::get_first_node_by_value(node* root, node* res_node,
...
node* pdrh::get_node_neg_by_value(node* root, vector<string> values) {
  ...
  // cout << "Node before removing time node: " <<
  // pdrh::node_to_string_prefix(root) << endl;
