...
// throws exception in case if one of the terminal modes is not a number
// evaluates the value of arithmetic expression
bool pdrh2box::check_zero_crossing(pdrh::node *expr, vector<box> boxes,
                                   box first, box last) {
  // comparison operators
  if (expr->value == ">=" || expr->value == ">" || expr->value == "=" ||
      expr->value == "<" || expr->value == "<=") {
    //        cout << "Beginning left: " <<
    //        pdrh::node_to_interval(expr->operands.front(), {boxes, first}) <<
    //        endl; cout << "Beginning right: " <<
    //        pdrh::node_to_interval(expr->operands.back(), {boxes, first}) <<
    //        endl; cout << "Beginning left-right: " <<
    //        pdrh::node_to_interval(expr->operands.front(), {boxes, first}) -
    //                                            pdrh::node_to_interval(expr->operands.back(),
    //                                            {boxes, first}) << endl;
    //        cout << "End left: " <<
    //        pdrh::node_to_interval(expr->operands.front(), {boxes, last}) <<
    //        endl; cout << "End right: " <<
    //        pdrh::node_to_interval(expr->operands.back(), {boxes, last}) <<
    //        endl; cout << "End left-right: " <<
    //        pdrh::node_to_interval(expr->operands.front(), {boxes, last}) -
    //                                           pdrh::node_to_interval(expr->operands.back(),
    //                                           {boxes, last}) << endl;
    return (pdrh2box::node_to_interval(expr->operands.front(), {boxes, first}) -
            pdrh2box::node_to_interval(expr->operands.back(), {boxes, first})) *
               (pdrh2box::node_to_interval(expr->operands.front(),
                                           {boxes, last}) -
                pdrh2box::node_to_interval(expr->operands.back(),
                                           {boxes, last})) <
           0;
  } else if (expr->value == "and") {
    bool res = true;
...
capd::interval pdrh2box::node_to_interval(pdrh::node *expr, vector<box> boxes) {
  ...
  if (expr->operands.size() == 0) {
    ...
    if (expr->value == "-infty") {
      return capd::interval(-numeric_limits<double>::max(),
    ...
    //        }
    //        CLOG(ERROR, "model") << "The number of operands can't be greater
    //        than 2";
    exit(EXIT_FAILURE);
  } else {
    if (expr->value == "+") {
...
box pdrh2box::get_psy_domain() {
  ...
  for (auto it = pdrh::syn_map.cbegin(); it != pdrh::syn_map.cend(); it++) {
    m.insert(make_pair(
        it->first,
        capd::interval(
            pdrh2box::node_to_interval(pdrh::var_map[it->first].first)
                .leftBound(),
...
box pdrh2box::init_to_box(vector<box> boxes) {
  ...
  for (pdrh::node *n : init_node->operands) {
    ...
    if ((pdrh::var_map.find(n->operands.front()->value) !=
         pdrh::var_map.end()) ||
        (pdrh::par_map.find(n->operands.front()->value) !=
         pdrh::par_map.end()) ||
        (pdrh::rv_map.find(n->operands.front()->value) != pdrh::rv_map.end()) ||
        (pdrh::dd_map.find(n->operands.front()->value) != pdrh::dd_map.end())) {
      b_map.insert(
          make_pair(n->operands.front()->value,
                    pdrh2box::node_to_interval(n->operands.back(), boxes)));
    } else if ((pdrh::var_map.find(n->operands.back()->value) !=
                pdrh::var_map.end()) ||
               (pdrh::par_map.find(n->operands.back()->value) !=
                pdrh::par_map.end()) ||
               (pdrh::rv_map.find(n->operands.back()->value) !=
                pdrh::rv_map.end()) ||
               (pdrh::dd_map.find(n->operands.back()->value) !=
                pdrh::dd_map.end())) {
      b_map.insert(
          make_pair(n->operands.back()->value,
