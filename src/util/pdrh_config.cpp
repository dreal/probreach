...
// setting max number of threads
  ...
  for (int i = 1; i < argc; i++) {
    ...
    // minimum reachability depth
      ...
      //            else if(global_config.reach_depth_min >
      //            global_config.reach_depth_max)
      //            {
      //                CLOG(ERROR, "config") << "Minimum reachaility depth
      //                cannot be smaller than the maximum one";
      //                exit(EXIT_FAILURE);
      //            }
    }
      ...
      while (!map_end) {
        if (is_flag(argv[i]) || is_drh(argv[i]) || is_pdrh(argv[i])) {
          ...
          if (is_flag(argv[i]) || is_drh(argv[i]) || is_pdrh(argv[i])) {
            ...
            // pdrh::push_nondet_partition_prec(var,
            // capd::interval(val_is.str(), val_is.str()));
    ...
    // solver binary
      ...
      //            global_config.solver_type =
      //            solver::detect_solver(global_config.solver_bin);
    }
    //        // solver binary
    ...
    ////            global_config.secondary_solver_type =
    ///solver::detect_solver(global_config.secondary_solver_bin);
    //        }
    // time variable name
      ...
      while (!map_end) {
        if (is_flag(argv[i]) || is_drh(argv[i]) || is_pdrh(argv[i])) {
          ...
          //                    if(find(global_config.time_var_name.begin(),
          //                    global_config.time_var_name.end(), argv[i]) ==
          //                    global_config.time_var_name.end())
          //                    {
      ...
      while (!map_end) {
        if (is_flag(argv[i]) || is_drh(argv[i]) || is_pdrh(argv[i])) {
          ...
          if (find(global_config.controller.controller_output.begin(),
                   global_config.controller.controller_output.end(), argv[i]) ==
    ...
    // controller output
      ...
      while (!map_end) {
        if (is_flag(argv[i]) || is_drh(argv[i]) || is_pdrh(argv[i])) {
          ...
          //                    if(find(global_config.controller.controller_input.begin(),
          //                    global_config.controller.controller_input.end(),
          //                    argv[i]) ==
          //                    global_config.controller.controller_input.end())
          //                    {
    ...
    //            while (!map_end)
    //            {
    //                if (is_flag(argv[i]) || is_drh(argv[i]) ||
    //                is_pdrh(argv[i]))
    //                {
    //                    map_end = true;
    //                    i--;
    ...
    //                    is_pdrh(argv[i]))
    //                    {
    //                        CLOG(ERROR, "config") << "partition precision for
    ...
    //                                make_pair(var,
    //                                capd::interval(val_is.str(),
    //                                val_is.str()).rightBound()));
    //                        i++;
    //                    }
    //                }
    //            }
    ...
    //            {
    //                cerr << "confidence of Chernoff bound should be within [0,
    //                1)"; exit(EXIT_FAILURE);
    //            }
    //        }
    //        // bayesian accuracy
    ...
    //            {
    //                cerr << "accuracy for Bayesian simulations should be
    //                positive"; exit(EXIT_FAILURE);
    //            }
    //        }
    //        // bayesian confidence
    ...
    //            {
    //                cerr << "confidence for Bayesian simulations should be
    //                within [0, 1)"; exit(EXIT_FAILURE);
    //            }
    //        }
    // qmc conf
      ...
      while (!map_end) {
        if (is_flag(argv[i]) || is_drh(argv[i]) || is_pdrh(argv[i])) {
          ...
          if (is_flag(argv[i]) || is_drh(argv[i]) || is_pdrh(argv[i])) {
            ...
            //                        istringstream val_is(argv[i]);
            //                        pdrh::push_prob_partition_prec(var,
            //                        capd::interval(val_is.str(),
            //                        val_is.str()));
...
void print_usage() {
  ...
  cout << "Usage:" << endl;
  cout << endl;
  cout << "	ProbReach <options> <file.pdrh/file.drh> <solver-options>"
       << endl;
  cout << endl;
  ...
  cout << "-c <double> - confidence value (default: " << global_config.conf
       << ")" << endl;
  //    cout << "--bayesian-acc <double> - half-length of the confidence
  //    interval in Bayesian estimations (default: " <<
  //    global_config.bayesian_acc << ")" << endl; cout << "--bayesian-conf
  //    <double> - confidence value in Bayesian estimations (default: " <<
  //    global_config.bayesian_conf << ")" << endl; cout << "--cross-entropy -
  //    enables Cross-Entropy algorithm (default: " <<
  //    global_config.cross_entropy_flag << ")" << endl; cout <<
  //    "--cross-entropy-term-arg <double> - termination argument (variance) for
  //    Cross-Entropy algorithm (default: " <<
  //    global_config.cross_entropy_term_arg << ")" << endl; cout <<
  //    "--chernoff-acc <double> - half-length of the confidence interval in
  //    Chernoff-Hoeffding method (default: " << global_config.chernoff_acc <<
  //    ")" << endl; cout << "--chernoff-conf <double> - confidence value in
  //    Chernoff-Hoeffding method (default: " << global_config.chernoff_conf <<
  //    ")" << endl;
...
bool is_flag(char* str) { return (str[0] == '-') && (str[1] == '-'); }
