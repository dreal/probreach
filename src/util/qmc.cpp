...
capd::interval algorithm::evaluate_qmc() {
  ...
  gsl_qrng *q = gsl_qrng_alloc(gsl_qrng_sobol,
                               static_cast<unsigned int>(pdrh::rv_map.size()));
...
#pragma omp critical
  ...
  cout << "[Psat, Punsat]= "
       << capd::interval(sat / global_config.qmc_sample_size,
                         (global_config.qmc_sample_size - unsat) /
                             global_config.qmc_sample_size)
       << endl;
  // Psat= Summ sat/n; Pusat=n-Summ usnat/n
  return capd::interval(
      sat / global_config.qmc_sample_size,
...
#pragma omp parallel
    while (CI <= Ca) {
      ...
      CLOG_IF(global_config.verbose_result, INFO, "algorithm")
          << "SOBOL SAMPLE :" << sobol_sample << endl;
      // sample from [x1_min,x1_max]*...*[xn_min,xn_max] after applying icdf
      box icdf_sample = rnd::get_icdf(sobol_sample);  //!!!!!
      CLOG_IF(global_config.verbose_result, INFO, "algorithm")
          << "ICDF sample :" << icdf_sample << endl;
      int res;

      res = decision_procedure::evaluate(paths, {icdf_sample},
                                         dreal::solver_bin, "");
...
#pragma omp critical
      {
        ...
        CLOG_IF(global_config.verbose_result, INFO, "algorithm")
            << "Number of SAT: " << sat2 << endl;
        CLOG_IF(global_config.verbose_result, INFO, "algorithm")
            << "Number of UNSAT: " << unsat2 << endl;
        CLOG_IF(global_config.verbose_result, INFO, "algorithm")
            << "Number of UNDET: " << undet2 << endl;
  ...
  // cout << "[Zsat, Zunsat]= " << capd::interval(Zresultsat,Zresultunsat) <<
  // endl; cout << "points===" << points << endl;
  for (int l = 1; l <= Zz; l++) {
    CLOG_IF(global_config.verbose_result, INFO, "algorithm")
        << l << "-test running points==" << pointsarray[l] << endl;
  }
...
#pragma omp critical
      {
        ...
        // CI = (global_config.qmc_acc / 2) / (pow(agrest_n, -0.5) *
        // sqrt(agrest_p * (1 - agrest_p)));
...
capd::interval algorithm::evaluate_rqmc_Will() {
  ...
  //        gsl_qrng *q2 = gsl_qrng_alloc(gsl_qrng_sobol, static_cast<unsigned
  //        int>(pdrh::rv_map.size()));
  //        // getting domain of random parameters
  //        map<string, capd::interval> sobol_domain_map2;
  //        for (auto &it : pdrh::rv_map) {
  //            sobol_domain_map2.insert(make_pair(it.first, capd::interval(0,
  //            1)));
  //        }
  //        box sobol_domain2(sobol_domain_map2);
...
#pragma omp critical
    ...
    Zresultunsat = Zresultunsat +
                   ((points - unsat2) + pow(Ca, 2) / 2) / (points + pow(Ca, 2));
    pointscount = pointscount + points;
    pointsarray[l] = points;
  }
  Zresultsat = Zresultsat / Zz;
...
#pragma omp critical
      {
        ...
        if (points == 1) {
          ...
          Var_Lambda = ((points + 1) * (points + 2)) /
                       (points * (sat2 + 1) * (points - sat2 + 1));
        } else {
        ...
        logit_u = exp(Lambda + Ca * sqrt(Var_Lambda)) /
                  (1 + exp(Lambda + Ca * sqrt(Var_Lambda)));
        logit_l = exp(Lambda - Ca * sqrt(Var_Lambda)) /
                  (1 + exp(Lambda - Ca * sqrt(Var_Lambda)));
...
capd::interval algorithm::evaluate_rqmc_Ans() {
  ...
  for (int l = 1; l <= Zz; l++) {
    ...
    // getting domain of random parameters
    map<string, capd::interval> sobol_domain_map2;
    map<std::__cxx11::basic_string<char, std::char_traits<char>,
                                   std::allocator<char>>,
        std::tuple<pdrh::node *, pdrh::node *, pdrh::node *,
                   pdrh::node *>>::iterator it;
...
#pragma omp critical
      {
        ...
        // Var_Lambda = ((points + 1) * (points + 2)) / (points * (sat2 + 1) *
        // (points - sat2 + 1));
        // cout << "Var_Lambda= " << Var_Lambda << endl;
        logit_u = exp(Lambda + Ca * sqrt(Var_Lambda)) /
                  (1 + exp(Lambda + Ca * sqrt(Var_Lambda)));
...
capd::interval algorithm::evaluate_Qint() {
  ...
  // Sobol generator
  gsl_qrng *q2 = gsl_qrng_alloc(gsl_qrng_sobol,
                                static_cast<unsigned int>(pdrh::rv_map.size()));
  map<string, capd::interval> sobol_domain_map2;
  for (auto &it : pdrh::rv_map) {
    sobol_domain_map2.insert(make_pair(it.first, capd::interval(0, 1)));
...
#pragma omp parallel
    ...
    for (int i = 0; i < 20; i++) {
      ...
      //            cout << "ss: " << ss << endl;
      //            cout << "Vector[" << i << "]" << endl;
      //            for (int f = 0; f < 2; f++)            //f<2 - 2- number of
      //            dimentions!!
      //                v.push_back(ss);
      //            for (auto &item : v)
      ...
      //                (pi < b) ? partTimes.push_back(a + 1) :
      //                partTimes.push_back(a);
      //                binaryIndex.push_back(static_cast<int &&>(floor(v[pi] *
      //                pow(2, partTimes[pi]))));
      //
      //                double sum;
      //                sum = 0;
...
#pragma omp critical
    ...
    for (double &fval : fvals) std::cout << "element - " << fval << "\n";
...
#pragma omp critical
      {
        ...
        // CLOG_IF(global_config.verbose_result, INFO, "algorithm") << "Number
        // of SAT: " << sat2 << endl; CLOG_IF(global_config.verbose_result,
        // INFO, "algorithm") << "Number of UNSAT: " << unsat2 << endl;
        // CLOG_IF(global_config.verbose_result, INFO, "algorithm") << "Number
        // of UNDET: " << undet2 << endl;

        // ressat2 = sat2 / points;
        // CLOG_IF(global_config.verbose_result, INFO, "algorithm") << "ressat:
        // " << ressat2 << endl; resunsat2 = (points - unsat2) / points;
        // CLOG_IF(global_config.verbose_result, INFO, "algorithm") <<
        // "resunsat: " << resunsat2 << endl;
        ...
        if (!CI_AC_result) {
          ...
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "ressat2: " << ressat2_AC << endl;
          resunsat2_AC = (points_AC - unsat2) / points_AC;
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "resunsat2: " << resunsat2_AC << endl;
        ...
        if (!CI_W_result) {
          ...
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "ressat2: " << ressat2_W << endl;
          resunsat2_W = (points_W - unsat2) / points_W;
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "resunsat2: " << resunsat2_W << endl;

          CI_W =
              (Ca * sqrt(points_W)) / (points_W + pow(Ca, 2)) *
        ...
        if (!CI_L_result) {
          ...
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "ressat2_L: " << ressat2_L << endl;
          resunsat2_L = (points_L - unsat2) / points_L;
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "resunsat2_L: " << resunsat2_L << endl;
          ...
          if (points_L == 1) {
            ...
            CLOG_IF(global_config.verbose_result, INFO, "algorithm")
                << "Lambda= " << Lambda << endl;
            Var_Lambda = ((points_L + 1) * (points_L + 2)) /
                         (points_L * (sat2 + 1) * (points_L - sat2 + 1));
          } else {
          ...
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "Lambda= " << Lambda << endl;
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "Var_Lambda= " << Var_Lambda << endl;
          logit_u = exp(Lambda + Ca * sqrt(Var_Lambda)) /
                    (1 + exp(Lambda + Ca * sqrt(Var_Lambda)));
          logit_l = exp(Lambda - Ca * sqrt(Var_Lambda)) /
                    (1 + exp(Lambda - Ca * sqrt(Var_Lambda)));
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "logit_u= " << logit_u << endl;
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "logit_l= " << logit_l << endl;
        ...
        if (!CI_ANS_result) {
          ...
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "ressat2_ANS: " << ressat2_ANS << endl;
          resunsat2_ANS = (points_ANS - unsat2) / points_ANS;
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "resunsat2_ANS: " << resunsat2_ANS << endl;

          Lambda_ANS = log((sat2 + 0.5) / (points_ANS - sat2 + 0.5));
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "Lambda_ANS= " << Lambda_ANS << endl;
          ...
          // (points - sat2 + 1));
          CLOG_IF(global_config.verbose_result, INFO, "algorithm")
              << "Var_Lambda_ANS= " << Var_Lambda_ANS << endl;
        ...
        if (CI_ARC_result == false) {
            ...
            cout << "arc_p= " << arc_p << endl;
            arc_l = pow(sin(asin(sqrt(arc_p))) - Ca / (2 * sqrt(points_ARC)),
        2); arc_u = pow(sin(asin(sqrt(arc_p))) + Ca / (2 * sqrt(points_ARC)),
        2); cout << "arc_l= " << arc_l << endl; cout << "arc_u= " << arc_u <<
        endl; CI_ARC = (arc_u - arc_l) / 2;
      ...
      allresults = CI_CLT_result && CI_AC_result && CI_W_result &&
                   CI_L_result && CI_ANS_result;  // && CI_ARC_result;
    } while (!allresults);
    ...
    points_W = points_W - 1;
    Zresultsat_W =
        Zresultsat_W + (sat2_W + pow(Ca, 2) / 2) / (points_W + pow(Ca, 2));
    Zresultunsat_W = Zresultunsat_W + ((points_W - unsat2_W) + pow(Ca, 2) / 2) /
                                          (points_W + pow(Ca, 2));
  ...
  cout << Zz
       << "-tests average running points number for CLT = " << pointscount_CLT
       << endl;

  cout << "[INTERVAL CLT]= "
       << capd::interval(Zresultsat_CLT - result, Zresultunsat_CLT + result)
       << endl;
  cout << "---------------------------------------------------------" << endl;
  // AC RESULTS
  ...
  cout << Zz
       << "-tests average running points number for AC = " << pointscount_AC
       << endl;

  cout << "[INTERVAL AC]= "
       << capd::interval(Zresultsat_AC - result_AC, Zresultunsat_AC + result_AC)
       << endl;
  cout << "---------------------------------------------------------" << endl;
  // W RESULTS
  ...
  cout << Zz << "-tests average running points number for W = " << pointscount_W
       << endl;

  cout << "[INTERVAL W]= "
       << capd::interval(Zresultsat_W - result_W, Zresultunsat_W + result_W)
       << endl;
  cout << "---------------------------------------------------------" << endl;
  // L RESULTS
  ...
  cout << Zz << "-tests average running points number for L = " << pointscount_L
       << endl;

  cout << "[INTERVAL L]= " << capd::interval(logit_l, logit_u) << endl;
  cout << "---------------------------------------------------------" << endl;
  // ANS RESULTS
  ...
  cout << Zz
       << "-tests average running points number for ANS = " << pointscount_ANS
       << endl;

  cout << "[INTERVAL ANS]= " << capd::interval(ANS_l, ANS_u) << endl;
  cout << "---------------------------------------------------------" << endl;
  // ARC RESULTS
  ...
  cout << Zz
       << "-tests average running points number for ARC = " << pointscount_ARC
       << endl;

  cout << "[INTERVAL ARC]= " << capd::interval(arc_l, arc_u) << endl;
  cout << "---------------------------------------------------------" << endl;
...
//    pdrh::get_all_paths(global_config.reach_depth_min,
//    global_config.reach_depth_max);
//
//    // initialize mu generator
//    gsl_qrng *m = gsl_qrng_alloc(gsl_qrng_sobol, static_cast<unsigned
//    int>(pdrh::rv_map.size()));
...
//
//    // initialize sobol generator
//    gsl_qrng *q = gsl_qrng_alloc(gsl_qrng_sobol, static_cast<unsigned
//    int>(pdrh::rv_map.size()));
//    // getting domain of random parameters
//    map<string, capd::interval> sobol_domain_map;
//    for (auto &it : pdrh::rv_map) {
...
//
//            // computing value of indicator function
//            switch (decision_procedure::evaluate_formal(paths,
//            {GPicdf_sample}, "")) {
//                // hybrid automata
//                case decision_procedure::SAT: {
//                    sat++;
...
//    cout << "Number of UNDET: " << undet << endl;
//
//    cout << "[Psat, Punsat]= " << capd::interval(sat /
//    global_config.qmc_sample_size,
//                                                 (global_config.qmc_sample_size
//                                                 - unsat) /
//                                                 global_config.qmc_sample_size)
...
#pragma omp critical
      {
        ...
        if (sat2 == 0) {
          LOarray = 0;
          UParray = gsl_cdf_beta_Pinv(1 - ((1 - conf) / 2), sat2 + 1,
                                      points - Sats[l]);
        } else if (sat2 == points) {
