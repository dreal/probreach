...
#pragma omp critical
  {
    for (int i = 0; i < global_config.qmc_sample_size; i++)
    {
      ...
      switch (decision_procedure::evaluate(
        paths, {icdf_sample}, dreal::solver_bin, ""))
      {
      // hybrid automata
...
capd::interval algorithm::evaluate_rqmc_Ans()
{
  ...
  for (int l = 1; l <= Zz; l++)
  {
    ...
    map<
      ...
      std::tuple<pdrh::node *, pdrh::node *, pdrh::node *, pdrh::node *>>::
      iterator it;
...
#pragma omp critical
      {
        ...
        if (!CI_AC_result)
        {
          ...
          points_AC++;
          // cout << "Ca==" << Ca << endl;
          if (global_config.qmc_acc / 2 >= CI_AC)
            CI_AC_result = true;
        }
        ...
        if (!CI_W_result)
        {
          ...
          points_W++;
          // cout << "Ca==" << Ca << endl;
          if (global_config.qmc_acc / 2 >= CI_W)
            CI_W_result = true;
        }
        ...
        if (!CI_L_result)
        {
          ...
          if (global_config.qmc_acc / 2 >= CI_L)
            CI_L_result = true;
        }
        ...
        if (!CI_ANS_result)
        {
          ...
          points_ANS++;
          // cout << "Ca==" << Ca << endl;
          if (global_config.qmc_acc / 2 >= CI_ANS)
            CI_ANS_result = true;
        }
        /*
                //ARC CALCULATING
...
#pragma omp critical
      {
        ...
        if (sat2 == 0)
        {
          LOarray = 0;
          UParray =
            gsl_cdf_beta_Pinv(1 - ((1 - conf) / 2), sat2 + 1, points - Sats[l]);
        }
        else if (sat2 == points)
        {
