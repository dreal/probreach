...
box rnd::get_random_sample(gsl_rng* r) {
  ...
  for (auto it = pdrh::rv_map.cbegin(); it != pdrh::rv_map.cend(); it++) {
    if (pdrh::distribution::uniform.find(it->first) !=
          ...
          pdrh2box::node_to_interval(
              ...
              gsl_rng_uniform(r) *
                  (pdrh2box::node_to_interval(
                   ...
                   pdrh2box::node_to_interval(
                       pdrh::distribution::uniform[it->first].first))));
    } else if (pdrh::distribution::normal.find(it->first) !=
          ...
          make_pair(it->first,
                    pdrh2box::node_to_interval(
                        ...
                        gsl_ran_gaussian_ziggurat(
                            r, pdrh2box::node_to_interval(
                                   pdrh::distribution::normal[it->first].second)
                                   .mid()
                                   .leftBound())));
    } else if (pdrh::distribution::exp.find(it->first) !=
               pdrh::distribution::exp.cend()) {
      edges.insert(make_pair(
          it->first,
          gsl_ran_exponential(
              r, pdrh2box::node_to_interval(pdrh::distribution::exp[it->first])
                     .mid()
                     .leftBound())));
    } else if (pdrh::distribution::gamma.find(it->first) !=
               pdrh::distribution::gamma.cend()) {
      edges.insert(make_pair(
          it->first,
          gsl_ran_gamma(r,
                        pdrh2box::node_to_interval(
                            pdrh::distribution::gamma[it->first].first)
                            .mid()
                            .leftBound(),
                        pdrh2box::node_to_interval(
...
box rnd::get_sobol_sample(gsl_qrng* q, box b) {
  ...
  for (auto it = b_edges.cbegin(); it != b_edges.cend(); it++) {
    edges.insert(make_pair(
        it->first, capd::interval(it->second.leftBound() +
                                  v[i] * capd::intervals::width(it->second))));
    i++;
...
box rnd::get_normal_random_sample(gsl_rng* r, box mu, box sigma) {
  ...
  for (auto it = pdrh::par_map.cbegin(); it != pdrh::par_map.cend(); it++) {
    if (it->second.first->value != it->second.second->value) {
      if (find(mu.get_vars().cbegin(), mu.get_vars().cend(), it->first) !=
              mu.get_vars().cend() &&
...
box rnd::get_beta_random_sample(gsl_rng* r, box alpha, box beta, box b) {
  ...
  for (auto it = b_map.cbegin(); it != b_map.cend(); it++) {
    double beta_dist_value =
        gsl_ran_beta(r, alpha.get_map()[it->first].leftBound(),
...
pair<box, box> rnd::update_beta_dist(vector<box> q, box domain, box prev_alpha,
                                     box prev_beta) {
  vector<box> q_norm;
  for (box b : q) {
    ...
    // cout << "initial box: " << b << " width: " << w << " left: " << l <<  "
    // normalized box: " << (b - l) / w << endl;
  }
...
pair<double, double> rnd::solve_beta_system(double c1, double c2,
                                            double prev_alpha,
                                            double prev_beta) {
...
box rnd::get_icdf(box b) {
  ...
  for (auto it = b_edges.cbegin(); it != b_edges.cend(); it++) {
    if (pdrh::distribution::uniform.find(it->first) !=
        pdrh::distribution::uniform.cend()) {
      double value =
          gsl_cdf_flat_Pinv(it->second.leftBound(),
                            pdrh2box::node_to_interval(
                                pdrh::distribution::uniform[it->first].first)
                                .leftBound(),
                            pdrh2box::node_to_interval(
                                ...
                                .leftBound());
      // value +=
      // pdrh::node_to_interval(pdrh::distribution::normal[it->first].first).leftBound();
...
box rnd::get_GPicdf(box b, box params) {
  ...
  for (auto it = b_edges.cbegin(); it != b_edges.cend(); it++) {
    if (pdrh::distribution::uniform.find(it->first) !=
        pdrh::distribution::uniform.cend()) {
      //            cout << "Distribution: " <<
      //            pdrh::node_to_string_infix(pdrh::distribution::uniform[it->first].first)
      //            << endl; cout << "Left bound: " <<
      //            pdrh2box::node_to_interval(pdrh::distribution::uniform[it->first].first,
      //            {params}).leftBound() << endl;
