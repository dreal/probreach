...
TEST(string_to_infix, normal)
{
    ...
    EXPECT_STREQ(string("(a+b)").c_str(), node("+",vector<node>{node("a"), node("b")}).to_infix().c_str());
}

TEST(string_to_prefix, normal)
{
    EXPECT_STREQ(string(" a").c_str(), node("a").to_prefix().c_str());
    EXPECT_STREQ(string("(+ a b)").c_str(), node("+",vector<node>{node("a"), node("b")}).to_prefix().c_str());
}
