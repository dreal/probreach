...
TEST(naive_ivp_solve, OK) {
  ...
  odes["Sx"] =
      new node("*", {new node("v0"), new node("cos", {new node("alpha")})});
  odes["Sy"] = new node(
      "-",
      {new node("*", {new node("v0"), new node("cos", {new node("alpha")})}),
...
TEST(naive_ivp_trajectory, OK) {
  ...
  for (auto it = init.begin(); it != init.end(); it++)
    init_map[it->first] = node_to_double(it->second);

  // defining the time limit and integration step
  double time = 5;
  double dt = 1e-4;
  // defining the guard
  //    pdrh::node* guard = new node("and", {new node(">", {new node("t"), new
  //    node("1e-6")}),
  //                                         new node("=", {new node("Sy"), new
  //                                         node("0")})});
  pdrh::node* guard = new node("<", {new node("Sy"), new node("0")});
  // solving the ODEs system
  vector<map<string, double>> traj =
      trajectory(odes, init_map, guard, time, dt);
...
TEST(naive_ivp_simulate, OK) {
  ...
  reset["v0"] = new node(
      "*", {new node("v0"),
            new node("dist_uniform", {new node("0.9"), new node("1.1")})});
  ...
  vector<vector<map<string, double>>> trajs =
      simulate({m}, init, depth, max_paths, dt);
